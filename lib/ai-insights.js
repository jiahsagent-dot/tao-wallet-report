...
// Map a current alpha-price to its decile bucket within a 6mo priceRange6mo
// distribution. The bucket label is appended to the top-positions prompt line
// so the LLM can cite the bucket verbatim instead of computing percentiles
// from {min,p10,median,p90,max} at inference time. Bucket boundaries match
// the MUST-cite conditions in PRICE_RANGE_6MO_HEURISTICS (iter 217).
function priceBucket(alpha, range) {
  if (!Number.isFinite(alpha) || !range) return null;
  const { min, p10, median, p90, max } = range;
  if (
    !Number.isFinite(min) ||
    !Number.isFinite(p10) ||
  ...
  if (alpha < min) return 'BELOW_MIN';
  if (alpha <= p10) return 'BELOW_P10';
  if (alpha <= median) return 'P10_TO_MEDIAN';
  if (alpha < p90) return 'MEDIAN_TO_P90';
  if (alpha <= max) return 'ABOVE_P90';
  return 'ABOVE_MAX';
}
...
function buildUserPrompt(report) {
  ...
  lines.push('');
  // Compute dossier first so the top-positions loop can append the
  // priceRange6mo decile bucket alongside the alpha-price field. The same
  // dossier array is reused for the per-position purpose block below.
  const dossier = dossierForPositions(p.top10);
  const dossierByNetuid = new Map(dossier.map((d) => [d.netuid, d]));
  ...
  for (const pos of p.top10.slice(0, 10)) {
    ...
    const bucket = priceBucket(
      pos.alphaPriceTao,
      dossierByNetuid.get(pos.netuid)?.priceRange6mo,
    );
    const alphaPx = Number.isFinite(pos.alphaPriceTao)
      ? ` · α=${pos.alphaPriceTao.toFixed(6)}τ${bucket ? ` · bucket=${bucket}` : ''}`
      : '';
    lines.push(
      `- sn${pos.netuid} ${pos.name}: ${pos.taoValue.toFixed(2)}τ (${pos.pctOfPortfolio.toFixed(1)}% of port) · 24h ${pct1d} · 7d ${pct7d}${alphaPx}`,
    );
  }
  lines.push('');
  ...
  if (dossier.length > 0) {
    ...
    for (const d of dossier) {
      ...
      const r = d.priceRange6mo;
      if (
        r &&
        Number.isFinite(r.min) &&
        Number.isFinite(r.p10) &&
        Number.isFinite(r.median) &&
        Number.isFinite(r.p90) &&
        Number.isFinite(r.max)
      ) {
        const asOf = r.asOf ? ` asOf=${r.asOf}` : '';
        const samples = Number.isFinite(r.samples) ? ` samples=${r.samples}` : '';
        lines.push(
          `  · 6mo alpha-price range (τ): min=${r.min.toFixed(6)} p10=${r.p10.toFixed(6)} median=${r.median.toFixed(6)} p90=${r.p90.toFixed(6)} max=${r.max.toFixed(6)}${samples}${asOf}`,
        );
      }
    }
    lines.push('');
