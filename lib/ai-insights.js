...
function buildUserPrompt(report) {
  ...
  if (sf?.available) {
    ...
    // Iter 137 — multi-window net-flow comparison. Surfaces the durability arc
    // (30d → 90d → 180d → 365d net flow tallies) so the §0 narrative reads
    // recent-flow shape against the long arc, not just the single-window
    // snapshot iter 123 emitted. Conditional render: only when mwnf is
    // available (i.e. ≥1 transfer in the 365d window) AND durability verdict
    // computes (i.e. four window buckets present).
    if (sf.multiWindowNetFlow?.available && sf.multiWindowDurability?.available) {
      const mw = sf.multiWindowNetFlow.points;
      const dur = sf.multiWindowDurability;
      const fmtRow = (p) => {
        const sign = p.netTao >= 0 ? '+' : '';
        return `${p.windowDays}d ${sign}${p.netTao.toFixed(1)}τ (${p.transferCount} txn${p.transferCount === 1 ? '' : 's'})`;
      };
      lines.push(`Multi-window net flow: ${mw.map(fmtRow).join(' · ')}.`);
      lines.push(`Durability: ${dur.verdict} — ${dur.verdictReason}.`);
    }
    lines.push('Staking flow is a behavioural overlay — it does NOT change ground-truth PnL (already accounted for in the formula). Read it relative to current portfolio % and cadence over the window, not absolute τ. If verdict is hands_off or passive, do not manufacture a flow narrative; if verdict is rebalancing, frame as activity not direction. When the multi-window durability verdict is present, read it as the LONG-ARC frame against which the single-window verdict is the RECENT slice — a 30d "accumulation" inside a 365d "sustained_distribution" is a recent reversal, not a confirmation, and the narrative must name that contrast rather than picking one window in isolation.');
    lines.push('');
  }
  ...
  if (yqv) {
    ...
    for (const row of yqv.rows) {
      ...
      lines.push(
        `  Plan APY (7d): ${planApy} · ${dirNote}${strayNote}${liftNote} · verdict: ${verdict}.`,
      );
