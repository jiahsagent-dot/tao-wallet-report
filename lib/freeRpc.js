...
// --- Historical stake + price legs (iter 220) ---
//
// Endpoint: https://archive.chain.opentensor.ai (same archive used by iter 219 for free leg).
// Reuses decodeStakeInfo + decodeU64LE + subnetPoolKey + accountStorageKey unchanged from current-state path.
// Only differences vs getColdkeyStakeAlpha / getSubnetPrices: archive endpoint +
//   state_call(..., blockHash) positional and state_queryStorageAt([keys], blockHash) positional.
//
// Iter 220 spike (scripts/probe-archive-stake.mjs) validated end-to-end on Jai coldkey
// 5EKFph3D… at CURRENT / 7d-ago / 30d-ago archive blocks:
//   - state_call("StakeInfoRuntimeApi_get_stake_info_for_coldkey", pubkey, blockHash) wraps cleanly
//     (runtime-API wraps at any block — metadata serves at archive depth per iter 281 finding)
//   - state_queryStorageAt batches SubnetTAO/SubnetAlphaIn at historical blockHash without issue
//   - latency observed: state_call ~240ms (current) / ~825ms (historical), prices batch ~225ms
//   - decoder + key builder reuse is 100% — no decode changes vs iter 165 / iter 167 current-state path
//   - composed stakeTao at 7d/30d: 1.706τ / 1.959τ vs current 1.772τ (sane drift driven by both
//     alpha-share evolution AND per-subnet AMM price evolution at historical blocks)
//
// Feature-flagged off: nothing in lib/report.js or lib/pnl.js imports these yet.
// iter 221 will compose getHistoricalColdkeyBalance(coldkey, secondsAgo) = free + reserved + stakeTao
// at the historical block, iter 222+ wires as candidate startingBalanceTao in pnlGroundTruth.

export async function getHistoricalColdkeyStakeAlpha(coldkey, blockHash, opts = {}) {
  const endpoint = opts.endpoint || ARCHIVE_ENDPOINT;
  const pubkeyHex = u8aToHex(decodeAddress(coldkey));
  const t0 = Date.now();
  const hex = await rpc('state_call', ['StakeInfoRuntimeApi_get_stake_info_for_coldkey', pubkeyHex, blockHash], { ...opts, endpoint });
  const latencyMs = Date.now() - t0;
  const entries = decodeStakeInfo(hex);
  const totalStakeAlpha = entries.reduce((s, e) => s + e.stakeAlpha, 0);
  return {
    ...
    totalStakeAlpha,
    latencyMs,
    source: 'archive-rpc:StakeInfoRuntimeApi',
    endpoint,
  };
}

export async function getHistoricalSubnetPrices(netuids, blockHash, opts = {}) {
  const endpoint = opts.endpoint || ARCHIVE_ENDPOINT;
  const targets = [...new Set(netuids.map(Number))].filter(n => Number.isInteger(n) && n > 0);
  const prices = { 0: 1.0 };
  const reserves = { 0: { taoIn: null, alphaIn: null, note: 'root: no AMM pool, 1:1 convention' } };
  if (targets.length === 0) return { prices, reserves, latencyMs: 0, source: 'archive-rpc:SubtensorModule', blockHash };
  const keys = [];
  for (const nu of targets) { keys.push(subnetPoolKey('SubnetTAO', nu)); keys.push(subnetPoolKey('SubnetAlphaIn', nu)); }
  const t0 = Date.now();
  const result = await rpc('state_queryStorageAt', [keys, blockHash], { ...opts, endpoint });
  const latencyMs = Date.now() - t0;
  const byKey = {};
  for (const [k, v] of (result?.[0]?.changes || [])) byKey[k.toLowerCase()] = v;
  for (const nu of targets) {
  ...
  return { prices, reserves, latencyMs, source: 'archive-rpc:SubtensorModule.SubnetTAO/SubnetAlphaIn', batchSize: keys.length, blockHash };
}

export async function getHistoricalColdkeyStakeTao(coldkey, blockHash, opts = {}) {
  const stake = await getHistoricalColdkeyStakeAlpha(coldkey, blockHash, opts);
  const netuids = [...new Set(stake.entries.map(e => e.netuid))];
  const { prices, reserves, latencyMs: priceLatencyMs, batchSize } = await getHistoricalSubnetPrices(netuids, blockHash, opts);
  let totalStakeTao = 0;
  const entries = stake.entries.map(e => {
    const price = prices[e.netuid] ?? 0;
    const stakeTao = e.stakeAlpha * price;
  ...
  return {
    ...
    latencyMs: { stake: stake.latencyMs, prices: priceLatencyMs, total: stake.latencyMs + priceLatencyMs },
    poolKeyBatchSize: batchSize,
    source: 'archive-rpc:StakeInfoRuntimeApi+SubtensorModule',
  };
}

// --- Composed historical balance (iter 221) ---
//
// Ties iter 219 free-leg (getHistoricalFreeBalance) + iter 220 stake-leg
// (getHistoricalColdkeyStakeTao) at a SINGLE SHARED archive blockHash so the
// two legs are time-aligned — critical for using this as candidate
// startingBalanceTao in pnlGroundTruth (iter 222+ wiring). A naive
// "fetch each leg with its own block lookup" would race the head between
// the two RPCs and give a sub-block-skewed snapshot.
//
// Internals:
//   1. getArchiveFinalizedHead() → head block number
//   2. blockNumberForSecondsAgo(head, secondsAgo) → target block number
//      (secondsAgo === 0 → target = head, i.e. live archive snapshot)
//   3. getArchiveBlockHash(target) → shared blockHash
//   4. Promise.all([getHistoricalFreeBalance, getHistoricalColdkeyStakeTao])
//      both at the same blockHash → 3 RPCs in parallel (1 free + 1 stake_call
//      + 1 price batch) — wall-clock ≈ slowest of the three (price batch ~225ms
//      or state_call ~735-825ms historical).
//   5. totalTao = freeTao + reservedTao + stakeTao  (matches Taostats
//      balance_total = free + reserved + staked decomposition surfaced by
//      iter 268's RAO-exact audit).
//
// Feature-flagged off: nothing in lib/report.js or lib/pnl.js imports this yet.
// iter 222+ wires as candidate startingBalanceTao in pnlGroundTruth replacing
// paid Taostats /api/account/history/v1.

// iter 228 — opts.anchorBlock overrides secondsAgo→block extrapolation. Set
// to the exact block_number from Taostats /api/account/history/v1 to land on
// the same chain block Taostats used for balance_staked. Closes the iter 227
// 12s-per-block extrapolation drift that pushed samples 8515 blocks (~28h)
// off and produced the +16.197 mτ residual masquerading as semantic
// divergence. See lessons_taostats_account_history_block_number_authoritative.md.
export async function getHistoricalColdkeyBalance(coldkey, secondsAgo = 0, opts = {}) {
  const endpoint = opts.endpoint || ARCHIVE_ENDPOINT;
  const t0 = Date.now();
  const tHead = Date.now();
  const headBlock = await getArchiveFinalizedHead({ ...opts, endpoint });
  const headLatencyMs = Date.now() - tHead;
  let blockNumber;
  let anchorSource;
  if (Number.isFinite(opts.anchorBlock) && opts.anchorBlock > 0) {
    blockNumber = opts.anchorBlock;
    anchorSource = 'taostats-block-number-exact';
  } else if (secondsAgo > 0) {
    blockNumber = blockNumberForSecondsAgo(headBlock, secondsAgo);
    anchorSource = 'extrapolated-12s-per-block';
  } else {
    blockNumber = headBlock;
    anchorSource = 'head';
  }
  const tHash = Date.now();
  const blockHash = await getArchiveBlockHash(blockNumber, { ...opts, endpoint });
  const hashLatencyMs = Date.now() - tHash;
  const tLegs = Date.now();
  const [free, stake] = await Promise.all([
    getHistoricalFreeBalance(coldkey, blockHash, { ...opts, endpoint }),
    getHistoricalColdkeyStakeTao(coldkey, blockHash, { ...opts, endpoint }),
  ]);
  const legsLatencyMs = Date.now() - tLegs;
  const freeTao = free.freeTao || 0;
  const reservedTao = free.reservedTao || 0;
  const stakeTao = stake.totalStakeTao || 0;
  const totalTao = freeTao + reservedTao + stakeTao;
  return {
    coldkey,
    secondsAgo,
    blockNumber,
    ...
    stakeTao,
    totalTao,
    freeRao: free.freeRao,
    reservedRao: free.reservedRao,
    totalStakeAlpha: stake.totalStakeAlpha,
    byNetuid: stake.byNetuid,
    prices: stake.prices,
    rpcCalls: 1 + 1 + 1 + 2, // head + hash + free + (stake_call + prices)
    latencyMs: {
      head: headLatencyMs,
      blockHash: hashLatencyMs,
      free: free.latencyMs,
      stake: stake.latencyMs.total,
      legsParallel: legsLatencyMs,
      total: Date.now() - t0,
    },
