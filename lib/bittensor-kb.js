...
// what was carried in prose across iters 279-287). Recomputed at module load
// so the block stays fresh as DOSSIER grows past 44 non-root entries.
...
- ${COHORT_STATS.counts['open-marketplace']} open-marketplace (2× ≤ spreadIQR < 6×, participation-based or open marketplace scoring ⇒ wider dispersion) — members ${COHORT_STATS.members['open-marketplace'].join(' / ')}. Top positions in open-marketplace subnets warrant extra scrutiny: alpha-price re-rate risk is materially wider than the competition-format median, so a 6mo p90 read on an open-marketplace holding carries more mark-to-market swing risk than the same p90 read on a competition-format holding. Cite the cohort explicitly when framing concentration or drawdown reads.
- ${COHORT_STATS.counts['spike-anomaly-flagged']} spike-anomaly-flagged (spreadIQR ≥ 6× OR spreadNaive/spreadIQR ratio > 10) — members ${COHORT_STATS.members['spike-anomaly-flagged'].join(' / ')}. Two structurally distinct failure modes flagged by the same rule: (i) spikeNaive/spikeIQR ratio > 10 = genuine spike-corrupted 6mo range (a brief parabolic event distorts the naive max/min while IQR remains merely elevated — treat priceRange6mo as unreliable for percentile framing, prefer YIELD_BANDS + EMISSION_ALIGNMENT_HEURISTICS instead); (ii) spreadIQR ≥ 6× with low naive/IQR ratio = genuine wide dispersion at the boundary of open-marketplace. Both cases warrant a "wider-than-typical alpha-price volatility" caveat when framing top-position narrative — the flag is the AI's permission to skip percentile-bucket precision in favour of directional reads.
