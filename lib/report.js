...
export async function pnlGroundTruth({ coldkey, days, taoPrice, usdAud, rows: preFetchedRows, balance: preFetchedBalance, rowsFetchError: preFetchedRowsError, balanceFetchError: preFetchedBalanceError, forceShadow = false }) {
  ...
  // Iter 169 — shadow-mode free-API balance. When FREE_PNL_SHADOW=1, ALSO
  // compute total_tao via lib/freePnl.js (3 RPC calls to finney HTTPS, zero
  // Taostats cost) and surface alongside Taostats under pnlGroundTruth
  // .shadowFreeApi. NOT a replacement — pure observability so iter 170+ can
  // measure drift in production conditions across all coldkeys. Wrapped in
  // a 5s timeout + catch-all because this is a load-bearing buildReport
  // path and the free-API call must NEVER throw out to pnlGroundTruth().
  // Iter 170 — forceShadow opt threaded from buildReport ← /api/report?shadow=1
  // lets a single request activate the shadow path without flipping the
  // process-wide FREE_PNL_SHADOW env (no Vercel env toggle, no cleanup, no
  // concurrent-request contamination). Either gate (per-request or env-wide)
  // is enough — the env stays as the "always on" knob for telemetry sweeps.
  let shadowFreeApi = null;
  if (forceShadow === true || process.env.FREE_PNL_SHADOW === '1') {
    ...
    try {
      ...
      const driftTao = currentPortfolioTao - r.totalTao;
