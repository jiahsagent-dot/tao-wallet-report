...
export async function pnlGroundTruth({ coldkey, days, taoPrice, usdAud, rows: preFetchedRows, balance: preFetchedBalance, rowsFetchError: preFetchedRowsError, balanceFetchError: preFetchedBalanceError, forceShadow = false }) {
  ...
  const runArchiveStartingShadow = async (firstSnapshotDate, firstSnapshotTimestamp) => {
    ...
    try {
      const r = await Promise.race([
        getHistoricalColdkeyBalance(coldkey, alignedSecondsAgo),
        new Promise((_, rej) =>
      ...
      return {
        ...
        alignedSecondsAgo,
        alignmentMethod: 'taostats-firstSnapshotDate-eod',
        rpcCalls: r.rpcCalls,
        latencyMs: r.latencyMs,
        wallClockMs: Date.now() - t0,
        source: r.source,
  ...
  let archiveStartingBalance = await runArchiveStartingShadow(firstSnapshotDate, firstSnapshotTimestamp);
