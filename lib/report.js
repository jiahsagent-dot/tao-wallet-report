...
export async function pnlGroundTruth({ coldkey, days, taoPrice, usdAud, rows: preFetchedRows, balance: preFetchedBalance, rowsFetchError: preFetchedRowsError, balanceFetchError: preFetchedBalanceError, forceShadow = false }) {
  ...
  // starting-balance leg in parallel with the existing current-balance shadow
  // above. Mirrors iter 172/207 pattern: race a 5s timeout, fail-open into
  // {ok:false,error}, surface drift telemetry on the return so future iters
  // (223+) can flip the flag and replace the paid Taostats
  // /api/account/history/v1 startingBalanceTao read. Default OFF — opt-in via
  // ARCHIVE_STARTING_SHADOW=1 so latency budget on every report isn't blown
  // until parity is validated across all 4 monitored coldkeys.
  //
  // Caveat: archive blockHash is positioned at days*86400 seconds back from
  // archive head (via the iter 219 blockNumberForSecondsAgo / 12.545s constant
  // baked into freeRpc.js). Taostats /api/account/history/v1 returns daily
  // EOD snapshots, so firstSnapshotDate may land up to ~24h off the archive
  // sample point. The drift telemetry surfaces the alignment offset so a
  // downstream consumer can attribute (alignment_offset, real_drift) instead
  // of conflating the two. Per lessons_bittensor_scale_u64_balance.md the
  // archive total = free + reserved + stake at the block hash is RAO-exact.
  const archiveStartingEnabled = process.env.ARCHIVE_STARTING_SHADOW === '1';
  const archiveStartingT0 = archiveStartingEnabled ? Date.now() : 0;
  const archiveStartingPromise = archiveStartingEnabled
    ? Promise.race([
        getHistoricalColdkeyBalance(coldkey, days * 86400),
        new Promise((_, rej) =>
        ...
        .then((r) => ({
          ...
          rpcCalls: r.rpcCalls,
          latencyMs: r.latencyMs,
          wallClockMs: Date.now() - archiveStartingT0,
          source: r.source,
        }))
        .catch((err) => ({
          ok: false,
  ...
  if (!rows.length || !balance) {
    ...
    const archiveStartingBalance = await archiveStartingPromise;
    return {
      available: false,
      reason,
      ...(error ? { error } : {}),
  ...
  if (startingBalanceTao == null) {
    ...
    const shadowFreeApi = await shadowPromise;
    const archiveStartingBalance = await archiveStartingPromise;
    return { available: false, reason: 'no_balance_snapshots', shadowFreeApi, archiveStartingBalance };
  }

  const currentPortfolioTao = balance.totalTao;

  // Iter 222 (auto-loop iter 285) — await archive-node starting-balance shadow
  // and compute drift vs Taostats startingBalanceTao. Day-boundary alignment
  // (archive sample at days*86400s back, Taostats first EOD snapshot at
  // firstSnapshotDate) is surfaced in alignmentSecondsOff so consumers can
  // attribute (alignment, real-rao-drift) instead of conflating. Defaults
  // OFF until parity validated on all 4 monitored coldkeys (env
  // ARCHIVE_STARTING_SHADOW=1 to opt in). NEVER replaces startingBalanceTao
  // on the production critical path — strictly observational this iter.
  let archiveStartingBalance = await archiveStartingPromise;
  if (archiveStartingBalance?.ok && Number.isFinite(archiveStartingBalance.totalTao)) {
    ...
    const firstSnapshotMs = firstSnapshotDate ? new Date(firstSnapshotDate).getTime() : null;
    const archiveSampleMs = Date.now() - (archiveStartingBalance.secondsAgo || 0) * 1000;
    const alignmentSecondsOff = firstSnapshotMs
      ? Math.round((archiveSampleMs - firstSnapshotMs) / 1000)
      : null;
