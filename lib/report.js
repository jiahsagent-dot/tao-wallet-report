...
export async function buildReport(coldkey) {
  ...
  const trendHint = (() => {
    ...
    // Soft-omit if either window is essentially flat (chip would be noise).
    ...
    if (sameSign && pct7d > 0) return { kind: 'rally', emoji: '📈', label: 'Week-long rally', pct24h, pct7d };
    if (sameSign && pct7d < 0) return { kind: 'bleed', emoji: '📉', label: 'Week-long bleed', pct24h, pct7d };
    if (oppSign && pct24h > 0) return { kind: 'bounce', emoji: '↩️', label: 'Bounce off weekly low', pct24h, pct7d };
    if (oppSign && pct24h < 0) return { kind: 'pullback', emoji: '🔻', label: 'Pullback in uptrend', pct24h, pct7d };
    return null;
  })();
