...
import {
  ...
  getApyFor,
  getSubnetScreener,
  getScreenerFromTaostatsPool,
  getTaxReport,
  getTaxReportRange,
  getLatestBalance,
  getValidatorYield,
  getBalanceHistory,
  getTransferAggregateFree,
} from './taostats.js';
import { getColdkeyBalance } from './freePnl.js';
import { getHistoricalColdkeyBalance } from './freeRpc.js';
...
export async function portfolio({ holdings, screener, taoPrice, usdAud }) {
  const positions = aggregateHoldings(holdings).map((pos) => {
    ...
    return {
      ...
      // iter 251 — per-subnet 24h gross trade volume in τ (tao.app screener
      // total_volume_tao_1d, already used in §6 broader). Threaded onto the
      // §1 top10 position map so the row can render a liquidity / exit-friction
      // chip without a second pass over screener.byNetuid on the client.
      // Single-snapshot field; null when subnet missing from screener or
      // total_volume_tao_1d is absent/non-finite. No new control flow, no
      // closure-captured state — iter 239 IIFE ReferenceError class doesn't
      // apply by construction.
      volTao24h: (() => {
        const v = sn.total_volume_tao_1d;
  ...
  //
  // iter 194 — IMPORTANT SNAPSHOT CAVEAT. emission_pct is a per-epoch
  // snapshot, not a sustained metric. Yuma consensus rotates validator
  // weight per ~72min tempo (360 blocks); at any instant ~90/128 subnets
  // show 0 (rotation), the other ~38 share emission. Over 24h most active
  // subnets receive some share. The "starved_subnet_heavy" verdict below
  // therefore reads as "starved IN THIS EPOCH SNAPSHOT", not "structurally
  // starved subnet". Future iter could compute a rolling-average from
  // Taostats history to get a sustained signal; for now the §0 narrative
  // qualifies the read and the iter 194 chip in Report.jsx adds an
  // "(epoch)" suffix so users don't read instantaneous as permanent.
  ...
  const emissionAlignment = (() => {
    ...
    let effectiveEmissionAbsorption = 0;
    // iter 239 — position-count axis alongside the existing TAO-weighted SHARE
    // axis. Shares answer "how much of my TAO is exposed to high-emission
    // subnets"; counts answer the orthogonal "how concentrated vs spread is
    // that exposure" question. Same N=11 wallet can read 60% high-emission /
    // 2 positions (concentrated bet) vs 60% high-emission / 9 positions
    // (broad coverage) — same verdict, materially different structural read.
    // Surfaced to §1 UI subtitle (this iter) and available for §0 prompt or
    // future heuristic blocks without further plumbing.
    let highEmissionPositionCount = 0;
    let zeroEmissionPositionCount = 0;
    const seenNetuids = new Set();
    for (const p of positionsWithEmission) {
      weightedEmissionNum += p.taoValue * p.emissionPct;
      effectiveEmissionAbsorption += p.taoValue * (p.emissionPct / 100);
      if (p.emissionPct >= HIGH_EMISSION_PCT) {
        highEmissionTao += p.taoValue;
        highEmissionPositionCount += 1;
      }
      if (p.emissionPct === 0) {
        zeroEmissionTao += p.taoValue;
        zeroEmissionPositionCount += 1;
      }
    ...
    const emissionPositionCount = positionsWithEmission.length;
    ...
    return {
      ...
      zeroEmissionShare,
      coveredEmissionPct,
      effectiveEmissionAbsorption,
      highEmissionPositionCount,
      zeroEmissionPositionCount,
      emissionPositionCount,
...
export async function pnlGroundTruth({ coldkey, days, taoPrice, usdAud, rows: preFetchedRows, balance: preFetchedBalance, rowsFetchError: preFetchedRowsError, balanceFetchError: preFetchedBalanceError, forceShadow = false }) {
  // Iter 172 — kick off shadow path IN PARALLEL with Taostats leg. Previously
  // shadow ran AFTER Taostats parsing, so any rows/balance failure took the
  // early-return below BEFORE shadow could execute — that's why the iter 171
  // 7-coldkey sweep got 6/7 http:500 (Taostats /price+/coldkey_alpha_shares
  // 429s blocked telemetry for the exact upstream we want to escape). Now
  // shadowPromise is kicked off here, awaited at each return path, and merged
  // in regardless of Taostats outcome. Drift only computed on the success
  // path where canonical currentPortfolioTao exists.
  // Iter 204 — flip the shadow path ON by default. Was env-gated on
  // FREE_PNL_SHADOW=1 since iter 168; iter 172 7-coldkey sweep validated the
  // parallel kickoff handles every Taostats failure mode without latency drag
  // (5s race timeout, .catch swallows every throw into {ok:false,error}).
  // The iter-180 canonical-graduation milestone only fires when
  // shadowFreeApi.ok=true, so leaving shadow off in prod means the free-API
  // safety net never actually rescues the user-facing experience during
  // Taostats outages (e.g. today's /api/account/latest/v1 + /api/price/latest/v1
  // intermittent failures surfaced in iter 166's followups). Default-on
  // enables the graduation. Set FREE_PNL_SHADOW=0 to explicitly opt out.
  const shadowEnabled = forceShadow === true || process.env.FREE_PNL_SHADOW !== '0';
  const shadowT0 = shadowEnabled ? Date.now() : 0;
  const shadowPromise = shadowEnabled
    ? Promise.race([
        getColdkeyBalance(coldkey),
        new Promise((_, rej) =>
          setTimeout(() => rej(new Error('FREE_PNL_SHADOW timeout 5s')), 5000),
        ),
      ])
        .then((r) => ({
          ok: true,
          totalTao: r.totalTao,
          freeTao: r.freeTao,
          reservedTao: r.reservedTao,
          stakeTao: r.stakeTao,
          stakeAlpha: r.stakeAlpha,
          rpcCalls: r.rpcCalls,
          latencyMs: r.latencyMs,
          wallClockMs: Date.now() - shadowT0,
          source: r.source,
        }))
        .catch((err) => ({
          ok: false,
  ...
  // Iter 265 — Priority #1 shadow telemetry (Option Y). Parallel kickoff of the
  // unwindowed /api/transfer/v1 aggregator (getTransferAggregateFree). Iter 263
  // proved this captures wallet-birth seed transfers (Mum_mantat GREEN vs paid,
  // 0 mτ delta) but misses stake-unlock rows (from_address=null on paid tax/v1,
  // invisible to /api/transfer/v1); iter 264 ruled out separability of those 4
  // rows in free /api/delegation/v1. Shadow-only surface — never promoted to
  // canonical currentPortfolioTao or transferInTao/transferOutTao — feeds the
  // shadowFreeTaxV2 observability field on the pnlGroundTruth return so future
  // iters can monitor drift of the birth-transfer + stake-unlock coverage gaps
  // in prod without changing user-visible PnL. FREE_TAX_SHADOW_V2=0 opts out.
  const shadowFreeTaxV2Enabled = process.env.FREE_TAX_SHADOW_V2 !== '0';
  const shadowFreeTaxV2T0 = shadowFreeTaxV2Enabled ? Date.now() : 0;
  const shadowFreeTaxV2Promise = shadowFreeTaxV2Enabled
    ? Promise.race([
        getTransferAggregateFree(coldkey),
        new Promise((_, rej) =>
          setTimeout(() => rej(new Error('FREE_TAX_SHADOW_V2 timeout 15s')), 15000),
        ),
      ])
        .then((r) => ({
          ok: true,
          transferInTao: r.transferInTao,
          transferOutTao: r.transferOutTao,
          transferCount: r.transferCount,
          wallClockMs: Date.now() - shadowFreeTaxV2T0,
          source: 'free:transfer/v1:unwindowed',
        }))
        .catch((err) => ({
          ok: false,
          error: err.message || String(err),
  ...
  // Iter 222 (autonomous-loop iter 285) — shadow-wire archive-node historical
  // starting-balance leg. Iter 224 (auto-loop iter 287) — REFACTOR: archive
  // call now deferred until AFTER Taostats /api/account/history/v1 returns so
  // sample point can be aligned to firstSnapshotDate EOD instead of days*86400
  // back from head. Iter 223 sweep found a uniform +7.33h alignment-window
  // drift caused by archive sample (now - 30d) vs Taostats EOD daily snapshot
  // (23:59:48 UTC). Iter 224 aligned re-sweep collapsed residual_seconds_off
  // from 26,388s to 1-2s on all 4 monitored coldkeys (Jai subnets / Jai mantat
  // / Mum subnets / Mum mantat) — free + reserved legs RAO-exact. Residual
  // 5-48 mRAO stake-leg drift remains with mixed signs, isolated as a separate
  // alpha-share→tao valuation parity problem queued for iter 225+.
  //
  // Trade-off: shadow now has Taostats /history as a hard dependency for its
  // alignment input. Fails open silently — null firstSnapshotDate → return
  // null (no badge, no error).
  //
  // Iter 234 (auto-loop iter 297) — Priority #1 GRADUATED. After 15 iters
  // (219→233) and 7 eliminated hypotheses (concentrated-outlier, Float64
  // round-then-sum, sn0-root-convention, sub-block AMM drift, per-hotkey
  // stake-base commission, commission-on-emission, daily-snapshot price-source
  // asymmetry) the residual is bounded at ±10 mτ on multi-AMM-subnet wallets
  // and sub-mτ on root-heavy baselines — well below decision-grade for the
  // free-tier PnL surface (~0.1% on a 5τ stake base). Default flipped on;
  // env=0 stays as the opt-OUT escape hatch.
  const archiveStartingEnabled = process.env.ARCHIVE_STARTING_SHADOW !== '0';
  // Taostats /api/account/history/v1 daily snapshots land at EOD UTC ~23:59:48
  // (lessons_taostats_account_endpoints + iter 223 finding). The `timestamp`
  // field on each row IS the EOD instant, so when we know firstSnapshotDate
  // we can derive secondsAgo to within block-time rounding (~12s).
  // iter 228 — accepts firstSnapshotBlockNumber alongside Date/Timestamp. When
  // present, archive samples the EXACT chain block Taostats used for
  // balance_staked (via getHistoricalColdkeyBalance opts.anchorBlock). Iter
  // 227 confirmed that the 12s/block extrapolation drifts ~28h over a 30d
  // window on Bittensor (slot-misses, validator stalls) and produced the
  // +16.197 mτ residual masquerading as semantic divergence. alignedSecondsAgo
  // is retained for badge logging only.
  const runArchiveStartingShadow = async (firstSnapshotDate, firstSnapshotTimestamp, firstSnapshotBlockNumber) => {
    if (!archiveStartingEnabled || !firstSnapshotDate) return null;
    const t0 = Date.now();
    // Prefer the EOD timestamp Taostats returned; fall back to "EOD of date"
    // if the row didn't expose .timestamp (defensive).
    const eodMs = firstSnapshotTimestamp
      ? new Date(firstSnapshotTimestamp).getTime()
      : (new Date(firstSnapshotDate).getTime() + (23 * 3600 + 59 * 60 + 48) * 1000);
    if (!Number.isFinite(eodMs)) {
      return { ok: false, error: 'unparseable firstSnapshotDate', wallClockMs: 0, source: 'archive-rpc:composed' };
    }
    const alignedSecondsAgo = Math.floor((Date.now() - eodMs) / 1000);
    // Defensive: skip wallets whose firstSnapshotDate is < 1 day old. Archive
    // head trails sub-block; alignedSecondsAgo close to 0 produces nonsense.
    if (alignedSecondsAgo < 86400) {
      return {
        ok: false,
        error: `snapshot too recent (alignedSecondsAgo=${alignedSecondsAgo}s)`,
        alignedSecondsAgo,
        wallClockMs: Date.now() - t0,
        source: 'archive-rpc:composed',
      };
    }
    const hasAnchorBlock = Number.isFinite(firstSnapshotBlockNumber) && firstSnapshotBlockNumber > 0;
    const callOpts = hasAnchorBlock ? { anchorBlock: firstSnapshotBlockNumber } : {};
    try {
      const r = await Promise.race([
        getHistoricalColdkeyBalance(coldkey, alignedSecondsAgo, callOpts),
        new Promise((_, rej) =>
          setTimeout(() => rej(new Error('ARCHIVE_STARTING_SHADOW timeout 5s')), 5000),
        ),
      ]);
      return {
        ok: true,
        ...
        secondsAgo: r.secondsAgo,
        alignedSecondsAgo,
        alignmentMethod: hasAnchorBlock
          ? 'taostats-block-number-exact'
          : 'taostats-firstSnapshotDate-eod',
  ...
  // Iter 147 — distinguish "tax_fetch_failed" (transient Vercel→Taostats 429
  // or timeout) from "no_tax_data" (Taostats genuinely has no rows). Iter 142
  // verify sweep on 2026-06-16 revealed that 6 of Jai's 7 pinned coldkeys
  // returned reason:"no_tax_data" from Vercel while direct VPS curl against
  // /api/accounting/tax/v1 returned 312 rows (Subnets) and 230 rows
  // (Mum_subnets). The lying reason was the `.catch(() => [])` here AND
  // upstream at the call site silently swallowing rate-limit errors as
  // "empty array." Now the reason reflects the actual cause.
  let rowsFetchError = preFetchedRowsError || null;
  // Iter 154 — mirror the iter 147 tax/v1 lying-catch fix for the balance call.
  // Previously `getLatestBalance(coldkey).catch(() => null)` silently swallowed
  // /api/account/latest/v1 429s and surfaced them as reason:"no_balance" — a
  // lie when the cause was a rate-limit, not a genuinely empty wallet. Mum_mantat
  // (5HbWj5vb…D1HL) hit this 2026-06-16 iter 152: tax_fetch SUCCEEDED, but the
  // balance call 429'd and the report misclassified as no_balance. Now the
  // reason reflects the actual cause via a new "balance_fetch_failed" branch.
  let balanceFetchError = preFetchedBalanceError || null;
  let rows;
  let balance;
  if (preFetchedRows !== undefined && preFetchedBalance !== undefined) {
    rows = preFetchedRows;
    balance = preFetchedBalance;
  } else {
    try {
      rows = await getTaxReport(coldkey, days);
    } catch (e) {
      rowsFetchError = String(e?.message || e);
      rows = [];
    }
    // Iter 258 — Priority #1 CANONICAL GRADUATION. freePnl.getColdkeyBalance
    // (finney HTTPS RPC, three-leg: free + reserved + stake) is now the
    // primary source of currentPortfolioTao when FREE_BALANCE_PRIMARY !== '0'
    // (unset or '1' = free-primary). Taostats getLatestBalance stays as the
    // .catch()-style fallback: if the RPC path throws (timeout, non-2xx,
    // decode failure) we transparently fall back to the paid endpoint. This
    // legacy no-preFetch path is rarely hit in prod (buildReport passes
    // preFetchedBalance) — the graduation for the buildReport-level path
    // lives at the balanceSeries pre-fetch below. FREE_BALANCE_PRIMARY=0 in
    // Vercel env restores legacy Taostats-primary without redeploy.
    const freeBalancePrimary = process.env.FREE_BALANCE_PRIMARY !== '0';
    balance = null;
    if (freeBalancePrimary) {
      try {
        const fp = await getColdkeyBalance(coldkey);
        balance = {
          totalTao: fp.totalTao,
          stakedTao: fp.stakeTao,
          freeTao: fp.freeTao,
          reservedTao: fp.reservedTao,
          snapshotAt: new Date().toISOString(),
          source: 'finney_rpc',
        };
      } catch (freeErr) {
        balanceFetchError = `free-primary failed: ${freeErr?.message || freeErr}`;
      }
    }
    if (!balance) {
      try {
        balance = await getLatestBalance(coldkey);
        if (freeBalancePrimary) balanceFetchError = null;
      } catch (e) {
        balanceFetchError = String(e?.message || e);
        balance = null;
      }
    }
  }
  ...
  if (!rows.length || !balance) {
    let reason;
    let error = null;
    if (rowsFetchError) { reason = 'tax_fetch_failed'; error = rowsFetchError; }
    else if (!rows.length) reason = 'no_tax_data';
    else if (balanceFetchError) { reason = 'balance_fetch_failed'; error = balanceFetchError; }
    else reason = 'no_balance';
    // Iter 172 — surface shadow telemetry even when Taostats failed. driftTao
    // /driftPct absent here (no canonical to compare against), but ok+totalTao
    // +freeTao+stakeTao prove free-API stack works for this coldkey.
    // Iter 224 — archive starting-balance shadow needs a firstSnapshotDate for
    // alignment; when Taostats failed entirely we have neither, so skip.
    const shadowFreeApi = await shadowPromise;
    const shadowFreeTaxV2 = await shadowFreeTaxV2Promise;
    return {
      available: false,
      reason,
      ...(error ? { error } : {}),
  ...
  // Iter 224 — capture the row.timestamp (EOD instant per
  // lessons_taostats_account_endpoints) alongside .date so the archive shadow
  // can align its sample to the exact snapshot moment, not 23:59:48 of the
  // string-parsed midnight.
  let firstSnapshotTimestamp = null;
  // iter 228 — capture row.block_number alongside .date/.timestamp so the
  // archive shadow can pass it as opts.anchorBlock to getHistoricalColdkeyBalance
  // and skip the 12s/block extrapolation that iter 227 confirmed drifts ~28h.
  let firstSnapshotBlockNumber = null;
  ...
  for (const r of rows) {
    ...
    if (t === 'transfer_in') {
      ...
      if (startingBalanceTao == null) {
        ...
        firstSnapshotTimestamp = r.timestamp || null;
        firstSnapshotBlockNumber = Number.isFinite(Number(r.block_number)) ? Number(r.block_number) : null;
      }
  ...
  if (startingBalanceTao == null) {
    // Iter 172 — surface shadow telemetry here too.
    // Iter 224 — no firstSnapshotDate available so archive alignment is impossible; skip.
    const shadowFreeApi = await shadowPromise;
    const shadowFreeTaxV2 = await shadowFreeTaxV2Promise;
    return { available: false, reason: 'no_balance_snapshots', shadowFreeApi, shadowFreeTaxV2, archiveStartingBalance: null };
  }

  const currentPortfolioTao = balance.totalTao;

  // Iter 222 (auto-loop iter 285) — archive-node historical starting-balance
  // shadow. Iter 224 (auto-loop iter 287) — alignment-aware: archive call
  // fired here AFTER Taostats /history returns and firstSnapshotDate /
  // firstSnapshotTimestamp are known, so the archive blockHash sample lands
  // at the Taostats EOD snapshot moment (±block-time rounding ~12s) instead
  // of (now - days*86400). Iter 223 sweep surfaced the uniform +7.33h
  // alignment-window drift this fixes. Iter 224 aligned re-sweep collapsed
  // residual to 1-2s; remaining 5-48 mRAO stake-leg drift is alpha-share→tao
  // valuation parity, separate iter 225+ concern. Default OFF until that
  // closes (env ARCHIVE_STARTING_SHADOW=1 to opt in). NEVER replaces
  // startingBalanceTao on the production critical path — observational only.
  let archiveStartingBalance = await runArchiveStartingShadow(firstSnapshotDate, firstSnapshotTimestamp, firstSnapshotBlockNumber);
  if (archiveStartingBalance?.ok && Number.isFinite(archiveStartingBalance.totalTao)) {
    const driftTao = startingBalanceTao - archiveStartingBalance.totalTao;
    const driftPct = startingBalanceTao > 0 ? driftTao / startingBalanceTao : null;
    // residualSecondsOff: archive sample point vs Taostats EOD snapshot.
    // Post-iter-224 this should be a handful of seconds (block-time rounding).
    const eodMs = firstSnapshotTimestamp
      ? new Date(firstSnapshotTimestamp).getTime()
      : (firstSnapshotDate ? new Date(firstSnapshotDate).getTime() + (23 * 3600 + 59 * 60 + 48) * 1000 : null);
    const archiveSampleMs = Date.now() - (archiveStartingBalance.secondsAgo || 0) * 1000;
    const alignmentSecondsOff = eodMs
      ? Math.round((archiveSampleMs - eodMs) / 1000)
      : null;
    // Iter 234 — bounded-noise tier added between match and drift. After 7
    // hypotheses eliminated (iter 219→233), the multi-AMM-subnet aggregation
    // residual is bounded at ±10 mτ; treat sub-10-mτ as parity within the
    // documented tolerance (green dot). 10–100 mτ stays drift (yellow);
    // anything else stays wide (red).
    const absDriftTao = Math.abs(driftTao);
    const status = absDriftTao < 0.001 ? 'match'
      : absDriftTao < 0.01 ? 'bounded-noise'
      : absDriftTao < 0.1 ? 'drift'
      : 'wide';
    archiveStartingBalance = {
      ...archiveStartingBalance,
      canonicalStartingTao: startingBalanceTao,
      driftTao,
      driftPct,
      status,
      alignmentSecondsOff,
  ...
  // Iter 172 — await shadow (kicked off at function top, ran in parallel with
  // Taostats leg) and merge in drift now that canonical currentPortfolioTao
  // is known. Refactored from iter 169/170 inline await so Taostats degraded
  // states (rows empty, balance missing) no longer short-circuit shadow.
  let shadowFreeApi = await shadowPromise;
  const shadowFreeTaxV2Raw = await shadowFreeTaxV2Promise;
  if (shadowFreeApi?.ok) {
    const driftTao = currentPortfolioTao - shadowFreeApi.totalTao;
    const driftPct = currentPortfolioTao > 0 ? driftTao / currentPortfolioTao : null;
    // Iter 206 (autonomous-loop iter 266) — decompose drift into free + stake legs.
    // Iter 208 (auto-loop iter 269) — FIX: iter 206 derived canonicalFreeTao as
    // (totalTao - stakedTao), which silently included balance_reserved and
    // fabricated a ~0.093τ "free-leg drift" on every wallet with a non-zero
    // reserved component (iter 268 NOTES validated this against substrate +
    // direct Taostats balance_free on 4 wallets — RAO-exact, no real drift).
    // Now using balance.freeTao + balance.reservedTao directly from Taostats
    // /api/account/history/v1 (added to getLatestBalance in iter 208) so the
    // free-leg drift correctly resolves to ~0 and the stake leg can speak
    // for itself (the actual /coldkey_alpha_shares-stale concern from
    // lessons_taostats_alpha_shares_stale.md).
    const canonicalStakeTao = Number(balance.stakedTao) || 0;
    const canonicalFreeTao = Number(balance.freeTao) || 0;
    const canonicalReservedTao = Number(balance.reservedTao) || 0;
    const freeDriftTao = canonicalFreeTao - (Number(shadowFreeApi.freeTao) || 0);
    const stakeDriftTao = canonicalStakeTao - (Number(shadowFreeApi.stakeTao) || 0);
    shadowFreeApi = {
      ...shadowFreeApi,
      canonicalTao: currentPortfolioTao,
      canonicalFreeTao,
      canonicalStakeTao,
      canonicalReservedTao,
      driftTao,
      driftPct,
      freeDriftTao,
      stakeDriftTao,
  ...
  return {
    ...
    // Iter 162 — provenance of currentPortfolioTao. When /api/account/history/v1
    // 429s, buildReport falls back to port.totalTao (alpha-share holdings ×
    // screener alpha prices) via a synthesized balance carrying
    // derivedFromDelegation:true. Surface this so the AI prompt + frontend can
    // mark the verdict as having a delegation-derived balance anchor (close
    // for pure-staking wallets, slightly under for mixed wallets that hold
    // free TAO outside delegation).
    // Iter 258 — new provenance value 'finney_rpc' when freePnl.getColdkeyBalance
    // supplied the current-value leg (FREE_BALANCE_PRIMARY canonical graduation).
    // Falls through to legacy 'history_api' when Taostats /history/v1 was
    // the source, and 'delegation_snapshot' when the iter-162 alpha-share
    // fallback fired.
    currentPortfolioSource: balance?.derivedFromDelegation
      ? 'delegation_snapshot'
      : (balance?.source === 'finney_rpc' ? 'finney_rpc' : 'history_api'),
    shadowFreeApi,
    // Iter 265 — Option Y shadow-only telemetry. transferIn/OutDriftTao are
    // (canonical − shadow). Iter 263 expected profile: transferOutDriftTao ~0
    // on all wallets (paid/free wire-shape parity); transferInDriftTao carries
    // the birth-transfer + stake-unlock coverage gap (positive = shadow under-
    // counts, matches iter 263 sign). Prod monitoring only, never affects the
    // shipped profitTao/returnPct fields above.
    shadowFreeTaxV2: shadowFreeTaxV2Raw?.ok
      ? {
          ...shadowFreeTaxV2Raw,
          canonicalTransferInTao: transferInTao,
          canonicalTransferOutTao: transferOutTao,
          transferInDriftTao: transferInTao - shadowFreeTaxV2Raw.transferInTao,
          transferOutDriftTao: transferOutTao - shadowFreeTaxV2Raw.transferOutTao,
        }
      : shadowFreeTaxV2Raw,
    archiveStartingBalance,
  };
...
export async function yieldSection({ holdings, screener }) {
  ...
  // iter 195 — read tao.app screener emission_pct for the same SNAPSHOT signal
  // surfaced in §1 (iter 193) and reframed in §1 (iter 194 (epoch) suffix +
  // emit-off-epoch tier). Same per-epoch caveat applies — see §1 portfolio()
  // block above for the Yuma rotation / snapshot semantics.
  const subnetEmissionPct = (nid) => {
    const raw = screener?.byNetuid?.[nid]?.emission_pct;
  ...
  const perPosition = holdings.map((h) => {
    ...
    return {
      ...
      emissionPct: subnetEmissionPct(h.netuid),
      apy: effectiveApy,
...
export async function buildAndCacheReport(coldkey, opts = {}) {
  const data = await buildReport(coldkey, opts);
  // Iter 170 — never cache shadow-mode responses. shadowFreeApi is per-request
  // telemetry; caching it would leak shadow data into subsequent plain calls
  // until TTL expiry and inflate the cached payload for non-shadow consumers.
  if (opts.shadow !== true) {
...
// opts.shadow=true forces the FREE_PNL_SHADOW path on for this single call
// (per-request telemetry; doesn't touch process env).
export async function buildReport(coldkey, opts = {}) {
  // Iter 155 — split the top-level fetch into two phases:
  //   Phase A (parallel, non-Taostats hosts): getSubnetScreener (tao.app),
  //     getUsdToAud (open.er-api.com). Different hosts, no collider.
  //   Phase B (sequential, Taostats endpoints, 75ms inter-request delay):
  //     getTaoPrice → getHoldings → getDelegationHistory. Same-host calls
  //     no longer fire in parallel from Vercel's shared egress IP.
  // iter 150 dispositive evidence: Mantat probe 500/6.7s with
  //   "/api/delegation/v1 → 429" and Mum_subnets 500/5.7s with
  //   "/api/dtao/coldkey_alpha_shares/latest/v1 → 429" — both wallets blocked
  // by per-IP throttle collisions between getHoldings + getDelegationHistory
  // running concurrently in the old Promise.all. Serialising the Taostats
  // bucket (only) keeps the cheap non-Taostats calls parallel so the
  // baseline-latency add is bounded to ~75ms × 2 + sequential-vs-max delta.
  // 2026-07-11 hotfix — tao.app invalidated TAOAPP_API_KEY server-side, so the
  // bare getSubnetScreener() throw in this Promise.all hard-500'd every report
  // ("tao.app screener → 403: Invalid API key"). Same degrade-don't-die
  // pattern as the iter 179/204 wraps. Fallback chain: tao.app screener →
  // Taostats /api/dtao/pool (full field parity except emission_pct, which all
  // consumers null-guard) → empty screener (alpha valuations degrade to 0 but
  // the report still renders).
  let screenerSource = 'tao.app';
  const [screener, usdAud] = await Promise.all([
    (async () => {
      try {
        return await getSubnetScreener();
      } catch (err) {
        console.warn(`[buildReport] tao.app screener failed: ${err?.message || err}`);
        try {
          const s = await getScreenerFromTaostatsPool();
          screenerSource = 'taostats-pool-fallback';
          return s;
        } catch (err2) {
          console.warn(`[buildReport] taostats pool fallback failed too: ${err2?.message || err2}`);
          screenerSource = 'unavailable';
  ...
  if (screenerSource === 'taostats-pool-fallback') {
    // The fallback just hit Taostats — give the per-IP throttle a beat before
    // Phase B's Taostats sequence starts (iter 155 collider discipline).
    await new Promise((r) => setTimeout(r, 75));
  }
  const taoPrice = await getTaoPrice();
  await new Promise((r) => setTimeout(r, 75));
  // Iter 204 — wrap getHoldings in try/catch. Bare await was the load-bearing
  // 500 path during Taostats /api/dtao/coldkey_alpha_shares/latest/v1 outages
  // (iter 166 verify rendered "⚠ Report build failed" for arbitrary coldkeys
  // because page-1 429 here bubbled straight through buildReport before the
  // shadow path could run inside pnlGroundTruth). On failure: holdings=[],
  // port.totalTao=0, but pnlGroundTruth still fires its parallel shadowPromise
  // (now default-on per iter 204) and the canonical-graduation gate below
  // surfaces shadowFreeApi.totalTao as the §1 headline number via
  // canonicalSource:"free-api-fallback". Identical defensive pattern to iter
  // 179 wrapping of getDelegationHistory; closes the orthogonal throttle
  // bucket (holdings vs delegation flap independently per iter 178).
  let holdingsFetchError = null;
  let holdings;
  try {
    holdings = await getHoldings(coldkey);
  } catch (err) {
    holdingsFetchError = String(err?.message || err);
    console.warn(
      `[buildReport] getHoldings threw for ${coldkey} — proceeding with empty holdings: ${holdingsFetchError}`,
    );
    holdings = [];
  }
  await new Promise((r) => setTimeout(r, 75));
  // Iter 179 — wrap getDelegationHistory in try/catch so a page-1 /api/delegation/v1
  // 429 doesn't collapse the entire report. Mum_subnets iter 178 sweep surfaced
  // the verbatim "Taostats /api/delegation/v1 → 429: Rate Limited" 500 body; the
  // existing iter 161 partial-data fallback only handles page>1 throws. On
  // failure here we proceed with history=[] (pnl section returns 0 spent/sold
  // → realisedTao=0, totalPnlTao=currentTao) and surface partial:true with
  // reason:"delegation_rate_limited" on the pnl result so consumers know the
  // realised PnL is incomplete. current_balance, emissions PnL, and shadow path
  // still ship intact. Endpoint-by-endpoint defensive fallback per iter 178
  // discovery: Taostats heavy endpoints flap independently; blanket fallback
  // (iter 177 CoinGecko for /price) won't close orthogonal throttle buckets.
  let historyFetchError = null;
  let history;
  try {
    history = await getDelegationHistory(coldkey);
  } catch (err) {
    historyFetchError = String(err?.message || err);
    console.warn(
      `[buildReport] getDelegationHistory threw for ${coldkey} — proceeding with empty history: ${historyFetchError}`,
    );
    history = [];
  }
  ...
  const currentFyStartYear = now.getUTCMonth() >= 6 ? now.getUTCFullYear() : now.getUTCFullYear() - 1;
  const fysToFetch = [currentFyStartYear - 1, currentFyStartYear];
  // Iter 154 — capture getBalanceHistory errors instead of swallowing as null.
  // The downstream `.catch(() => null)` was the second half of the lying-catch
  // pattern: pre-fetched balance was null both for genuinely-empty wallets
  // AND for transient 429s, and pnlGroundTruth had no way to distinguish.
  // Now the error is captured and threaded through so pnlGroundTruth can
  // surface reason:"balance_fetch_failed" with the actual error string.
  // Iter 159 — ROLLBACK of iter 158's chained-Promise pattern AND elimination
  // of the duplicate `/api/account/history/v1` call. Both getBalanceHistory
  // and getLatestBalance hit the SAME endpoint, so chaining them just spent
  // double the per-IP retry budget against the same throttle bucket — iter
  // 158 verify (2026-06-16T11:09Z) proved this: Subnets REGRESSED to
  // balance_fetch_failed (was matched ✓ since iter 149) and Mum_mantat still
  // tax_fetch_failed at 56.1s. Per lessons_taostats_account_endpoints.md the
  // /account/history/v1 endpoint is daily snapshots, so the LAST row of the
  // balance-history series is functionally equivalent to a separate
  // getLatestBalance call (same freshness, same source, same shape after
  // remapping `timestamp` → `snapshotAt`). One call replaces two; throttle
  // collision permanently eliminated.
  let latestBalanceError = null;
  const balanceSeriesP = getBalanceHistory(coldkey, 730).catch((e) => {
    latestBalanceError = String(e?.message || e);
    return [];
  });
  // Iter 258 — Priority #1 CANONICAL GRADUATION. Fire freePnl.getColdkeyBalance
  // in parallel with the Taostats balanceSeries fetch so we always have a
  // fallback ready. When FREE_BALANCE_PRIMARY !== '0' (default) and this call
  // succeeds, its result becomes latestBalance (currentPortfolioTao source).
  // getBalanceHistory still fires because taxYearSection needs the full 730d
  // series — this iter graduates the CURRENT-value leg only, not the
  // historical starting-balance leg (that's a separate iter 259+ concern).
  // Rollback: FREE_BALANCE_PRIMARY=0 in Vercel env, no redeploy required.
  const freeBalancePrimary = process.env.FREE_BALANCE_PRIMARY !== '0';
  const freeBalanceT0 = Date.now();
  const freeBalanceP = freeBalancePrimary
    ? getColdkeyBalance(coldkey)
      .then((r) => ({ ok: true, wallClockMs: Date.now() - freeBalanceT0, ...r }))
      .catch((err) => ({
        ok: false,
        error: err?.message || String(err),
        wallClockMs: Date.now() - freeBalanceT0,
      }))
    : Promise.resolve(null);
  const fyRowsBySy = new Map();
  const fyRowsErrorBySy = new Map(); // iter 147 — capture per-FY fetch error
  for (const sy of fysToFetch) {
    let rows = [];
    try {
      rows = await getTaxReportRange(
        coldkey, `${sy}-07-01`, `${sy + 1}-06-30`,
      );
    } catch (e) {
      fyRowsErrorBySy.set(sy, String(e?.message || e));
    }
    fyRowsBySy.set(sy, rows);
  }
  const balanceSeries = await balanceSeriesP;
  const freeBalance = await freeBalanceP;
  // Iter 159 — derive latestBalance from the LAST row of balanceSeries instead
  // of a separate getLatestBalance() call. getBalanceHistory's `out` array is
  // sorted ascending by timestamp (see lib/taostats.js:699), so `.at(-1)` is
  // the most recent daily snapshot — identical freshness to getLatestBalance.
  // Shape-match: balanceSeries row is {timestamp, totalTao, stakedTao} →
  // latestBalance is {stakedTao, totalTao, snapshotAt} so we remap.
  const lastSeriesRow = balanceSeries.length > 0
    ? balanceSeries[balanceSeries.length - 1]
    : null;
  let latestBalance = lastSeriesRow
    ? {
      stakedTao: lastSeriesRow.stakedTao,
      totalTao: lastSeriesRow.totalTao,
      // iter 209 — forward freeTao + reservedTao now that fetchBalanceHistory
      // carries them. Without this, pnlGroundTruth's canonicalFreeTao read of
      // balance.freeTao silently collapses to 0 (Number(undefined)||0),
      // recreating the exact bug iter 208 was supposed to fix.
      freeTao: lastSeriesRow.freeTao,
      reservedTao: lastSeriesRow.reservedTao,
      snapshotAt: lastSeriesRow.timestamp,
    }
    : null;

  // Iter 258 — Priority #1 CANONICAL GRADUATION. Prefer the freePnl RPC
  // reading over the Taostats /history/v1 last row when FREE_BALANCE_PRIMARY
  // !== '0' AND the RPC call succeeded. Field rename: freePnl.stakeTao →
  // balance.stakedTao (report.js downstream convention). On any freePnl
  // failure we silently keep the Taostats-derived latestBalance from
  // balanceSeries — the fallback is transparent and requires no user or
  // operator action. Iter 234+ shadow (default-on since 2026-06) proved
  // drift bounded at ±10 mτ (~0.02% on a 5τ base), well below decision-grade
  // for the ground-truth PnL surface.
  if (freeBalance?.ok && Number.isFinite(freeBalance.totalTao)) {
    latestBalance = {
      totalTao: freeBalance.totalTao,
      stakedTao: freeBalance.stakeTao,
      freeTao: freeBalance.freeTao,
      reservedTao: freeBalance.reservedTao,
      snapshotAt: new Date().toISOString(),
      source: 'finney_rpc',
      freeBalanceMeta: {
        wallClockMs: freeBalance.wallClockMs,
        rpcCalls: freeBalance.rpcCalls,
        legLatencyMs: freeBalance.latencyMs,
      },
    };
    // Free succeeded → don't propagate any Taostats /history/v1 error into
    // pnlGroundTruth's reason string; the ground-truth path has what it needs.
    latestBalanceError = null;
  }

  // Iter 162 — derive a fallback currentPortfolio from port.totalTao (computed
  // above from the coldkey's alpha-share holdings × screener alpha prices)
  // when /api/account/history/v1 429s leave balanceSeries empty. For pure-
  // staking wallets like Mantat (5CTRC3sQ…ArLn) where free TAO ≈ 0 this is
  // functionally equivalent to balance.totalTao; for mixed wallets it omits
  // free balance but is strictly better than reason:"balance_fetch_failed"
  // (no verdict, no portfolio anchor). Direct unblock for Mantat — iter 161
  // verify proved the 30s pagination budget cap broke the 504 timeout, but
  // verdict still didn't land due to the orthogonal /history/v1 throttle
  // bucket. The fallback closes that gap using data we already fetched, with
  // zero extra API calls. Tagged `derivedFromDelegation:true` so
  // pnlGroundTruth can surface currentPortfolioSource:"delegation_snapshot"
  // and the AI prompt KB can reflect the provenance.
  if (!latestBalance && Number.isFinite(port?.totalTao) && port.totalTao > 0) {
    latestBalance = {
      stakedTao: port.totalTao,
      totalTao: port.totalTao,
      snapshotAt: new Date().toISOString(),
      derivedFromDelegation: true,
    };
    // Clear the captured /history/v1 error so the report's pnlGroundTruth
    // doesn't surface reason:"balance_fetch_failed" — the fallback succeeded
    // even though the upstream call didn't. The original error is still
    // logged in balanceSeries=[] context for any downstream telemetry.
    latestBalanceError = null;
  }
  ...
  const trailingRowsError = fyRowsErrorBySy.get(currentFyStartYear) || null;
  ...
  const [p, gt, dd, ty] = await Promise.all([
    ...
    pnlGroundTruth({
      ...
      rowsFetchError: trailingRowsError,
      balanceFetchError: latestBalanceError,
      forceShadow: opts.shadow === true,
    }),
  ...
  // Iter 179 — surface delegation_rate_limited on the pnl result. When
  // historyFetchError is set the realised-PnL math saw an empty event stream,
  // so spentTao/soldTao/realisedTao are all 0 and totalPnlTao collapses to
  // currentTao. The flag lets §3/§4 consumers + AI insights gate any "realised
  // PnL" claims behind partial:true instead of presenting 0τ as ground truth.
  if (historyFetchError && p && typeof p === 'object') {
    p.partial = true;
    p.partialReason = 'delegation_rate_limited';
    p.partialDetail = historyFetchError;
  }
  ...
  // Iter 180 — priority #1 graduation milestone. When pnl.partial=true (Taostats
  // canonical PnL feed degraded — currently the /api/delegation/v1 429 path
  // surfaced by iter 178 and try/catch-wrapped by iter 179) AND
  // pnlGroundTruth.shadowFreeApi.ok=true, decorate the §1 portfolio with
  // canonicalSource:"free-api-fallback" + canonicalTao = shadowFreeApi.totalTao
  // so the UI can render a footnote ("free-API source — Taostats /delegation
  // rate-limited") under the headline number. The §1 portfolio.totalTao itself
  // already ships unchanged (computed from holdings × screener); the footnote
  // surfaces the parallel free-API number that downstream consumers (AI
  // insights, subscription email, future "current_balance" headline) can graduate
  // to as the canonical figure during Taostats flaps.
  // Iter 204 — extend iter 180's graduation trigger. Originally only fired
  // when pnl.partial=true (delegation 429). Now also fires when getHoldings
  // 429'd and we fell back to []. Without this gate, §1 would ship
  // totalTao=0 even though shadowFreeApi knows the wallet's canonical TAO.
  // canonicalReason now reflects which leg degraded (delegation vs holdings
  // vs both) so the UI footnote can stay precise.
  const holdingsRateLimited = !!holdingsFetchError;
  const partialPnl = p?.partial === true;
  const shadowOk = gt?.shadowFreeApi?.ok === true;
  if ((partialPnl || holdingsRateLimited) && shadowOk) {
    portClean.canonicalSource = 'free-api-fallback';
    portClean.canonicalTao = gt.shadowFreeApi.totalTao;
    portClean.canonicalReason = partialPnl && holdingsRateLimited
      ? 'holdings_and_delegation_rate_limited'
      : holdingsRateLimited
        ? 'holdings_rate_limited'
        : (p.partialReason || 'delegation_rate_limited');
  } else if (shadowOk && Number.isFinite(gt.shadowFreeApi.totalTao)) {
    // iter 205 (autonomous-loop iter 265) — Priority #1 user-visible. On
    // every HEALTHY report (no Taostats degradation), surface the parallel
    // substrate-RPC total + drift as a small badge so the user can SEE the
    // free-API verification running per page load. Today the existing
    // canonical-footnote (iter 180/204) only renders on Taostats flaps —
    // which means Priority #1 has been invisible 99% of the time despite
    // shipping cleanly. The data is already pre-computed at
    // pnlGroundTruth() line 333 (driftTao = currentPortfolioTao -
    // shadowFreeApi.totalTao). Status classifier mirrors bittensor-tracker
    // sweep parity terminology (lessons_bittensor_scale_u64_balance.md):
    //   match — |drift| < 0.001 τ (within RAO precision; same block)
    //   drift — |drift| < 0.1 τ   (small lag, normal between block + screener tick)
    //   wide  — |drift| ≥ 0.1 τ   (worth a tooltip read, often partial-holdings)
    // Mutually exclusive with canonical-graduation footnote above so we
    // never show two parallel-total disclosures on the same render.
    const driftAbs = Math.abs(Number(gt.shadowFreeApi.driftTao) || 0);
    const status = driftAbs < 0.001 ? 'match' : driftAbs < 0.1 ? 'drift' : 'wide';
    // Iter 206 (autonomous-loop iter 266) — per-leg attribution. Classify
    // which leg owns the gap (free | stake | both | null) using the same
    // 0.001τ RAO-precision threshold as the top-level status. This lets the
    // UI tooltip + future AI insights say "the drift is in the stake leg"
    // instead of just "5% drift" — proving Mum's gap traces to stale Taostats
    // /coldkey_alpha_shares not a substrate decode bug (iter 205 finding).
    const freeDriftAbs = Math.abs(Number(gt.shadowFreeApi.freeDriftTao) || 0);
    const stakeDriftAbs = Math.abs(Number(gt.shadowFreeApi.stakeDriftTao) || 0);
    let driftLeg = null;
    if (status !== 'match') {
      const freeHit = freeDriftAbs >= 0.001;
      const stakeHit = stakeDriftAbs >= 0.001;
      driftLeg = freeHit && stakeHit ? 'both' : stakeHit ? 'stake' : freeHit ? 'free' : null;
    }
    portClean.shadowVerified = {
      totalTao: gt.shadowFreeApi.totalTao,
      freeTao: gt.shadowFreeApi.freeTao,
      stakeTao: gt.shadowFreeApi.stakeTao,
      canonicalTao: gt.shadowFreeApi.canonicalTao,
      canonicalFreeTao: gt.shadowFreeApi.canonicalFreeTao,
      canonicalStakeTao: gt.shadowFreeApi.canonicalStakeTao,
      canonicalReservedTao: gt.shadowFreeApi.canonicalReservedTao,
      driftTao: gt.shadowFreeApi.driftTao,
      driftPct: gt.shadowFreeApi.driftPct,
      freeDriftTao: gt.shadowFreeApi.freeDriftTao,
      stakeDriftTao: gt.shadowFreeApi.stakeDriftTao,
      driftLeg,
      status,
      source: 'finney-substrate-rpc',
    };

    // iter 207 (autonomous-loop iter 267) — independent substrate cross-check.
    // When Priority #1 surfaces drift, the tao-wallet-report side has decoded
    // System.Account u64 SCALE via finney RPC once. bittensor-tracker.app's
    // /api/wallet-balance-rpc-sweep (autonomous-loop iter 262/264) ships an
    // INDEPENDENT substrate decode — separate codebase, separate RPC client,
    // separate SCALE decoder. Asking it for the same coldkey gives a second
    // witness: if both substrate decodes agree on freeRao but Taostats
    // diverges, the tooltip can confidently name Taostats as the outlier.
    // Skip on status="match" (no gap to investigate). Fail-open: any error
    // (timeout, network, parse, http) leaves crossCheck.ok=false and the
    // tooltip simply omits the third line. 2.5s timeout is generous since
    // sweep is <500ms warm / <2s cold.
    if (status !== 'match' && coldkey) {
      const ctrl = new AbortController();
      const timer = setTimeout(() => ctrl.abort(), 2500);
      try {
        const xRes = await fetch(
          `https://bittensor-tracker.app/api/wallet-balance-rpc-sweep?coldkeys=${encodeURIComponent(coldkey)}`,
          { signal: ctrl.signal, headers: { 'user-agent': 'tao-wallet-report/iter-207' } },
        );
        if (xRes.ok) {
          const xJson = await xRes.json();
          const xEntry = Array.isArray(xJson?.coldkeys) ? xJson.coldkeys[0] : null;
          const xRawFree = xEntry?.balance?.freeRao;
          const xFreeRao = xRawFree == null ? null : Number(xRawFree);
          const xFreeTao = Number.isFinite(xFreeRao) ? xFreeRao / 1e9 : null;
          const substrateFreeRao = Math.round(Number(gt.shadowFreeApi.freeTao) * 1e9);
          const canonicalFreeRao = Math.round(Number(gt.shadowFreeApi.canonicalFreeTao) * 1e9);
          const agreesSub = Number.isFinite(xFreeRao) &&
            Math.abs(xFreeRao - substrateFreeRao) < 1_000_000;
          const agreesTao = Number.isFinite(xFreeRao) &&
            Math.abs(xFreeRao - canonicalFreeRao) < 1_000_000;
          portClean.shadowVerified.crossCheck = {
            ok: true,
            freeRao: Number.isFinite(xFreeRao) ? xFreeRao : null,
            freeTao: xFreeTao,
            agreesWithSubstrate: agreesSub,
            agreesWithTaostats: agreesTao,
            source: 'bittensor-tracker-sweep',
          };
        } else {
          portClean.shadowVerified.crossCheck = {
            ok: false,
            source: 'bittensor-tracker-sweep',
            error: `http_${xRes.status}`,
          };
        }
      } catch (e) {
        portClean.shadowVerified.crossCheck = {
