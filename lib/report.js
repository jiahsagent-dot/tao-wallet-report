...
// §1 PORTFOLIO
//
// iter 3.5: headline `totalTao` is now sourced from `getLatestBalance(coldkey).totalTao`
// (account/history/v1.balance_total) when available, NOT from the sum of alpha
// subnet positions. Reason: alpha-position-sum silently dropped root-staked TAO
// + free TAO, understating root-heavy wallets by 100% and balanced wallets by
// ~50%. See lib/AUDIT.md (audit/numbers-iter3) for the spot-check that
// documented the delta against live Taostats data.
//
// Root staking and liquid balance are now surfaced as synthetic §1 rows
// (netuid 0 = "Root", netuid -1 = "Liquid") so the table is honest about the
// rest of the wallet. emissionAlignment math keeps its old denominator
// (alpha-only sum) so its verdict thresholds (60% high-emission share etc.)
// still mean what iter 125 designed them to mean.
export async function portfolio({ holdings, screener, taoPrice, usdAud, latestBalance }) {
  // Drop netuid 0 from the alpha-shares feed: root stake comes through the
  // alpha-shares endpoint with α-price 0 (not a real subnet token), which
  // renders as a phantom 'Subnet 0 / 6.73 / 0.000000 / 0.00 τ' ghost row
  // next to our synthetic ROOT badge row. The synthetic row from
  // latestBalance.rootStakedTao is the source of truth.
  const alphaPositions = aggregateHoldings(holdings)
    .filter((pos) => pos.netuid !== 0)
    .map((pos) => {
  ...
  // Denominator preserved for emissionAlignment so its verdict thresholds
  // (iter 125) keep their semantics.
  const totalAlphaTao = alphaPositions.reduce((a, p) => a + p.taoValue, 0);

  // Synthesise root + liquid rows from balance_total when available so the
  // headline reflects the wallet's actual TAO holdings.
  const syntheticRows = [];
  if (latestBalance) {
    if (latestBalance.rootStakedTao > 0) {
      syntheticRows.push({
        netuid: 0,
        name: 'Root',
        alphaHeld: latestBalance.rootStakedTao, // root is already in TAO
        alphaPriceTao: 1,
        taoValue: latestBalance.rootStakedTao,
        usdValue: latestBalance.rootStakedTao * taoPrice,
        emissionPct: null,
        pct1d: null,
        pct7d: null,
        pct1m: null,
    ...
    const liquid = (latestBalance.freeTao || 0) + (latestBalance.reservedTao || 0);
    if (liquid > 0) {
      syntheticRows.push({
        netuid: -1,
        name: 'Liquid',
  ...
  const positions = [...alphaPositions, ...syntheticRows];

  // Headline total: prefer live balance_total when available so root + liquid
  // are reflected; fall back to alpha-sum if Taostats latest-balance fetch failed.
  const totalTao =
    latestBalance && Number.isFinite(latestBalance.totalTao) && latestBalance.totalTao > 0
      ? latestBalance.totalTao
      : positions.reduce((a, p) => a + p.taoValue, 0);
  ...
  const emissionAlignment = (() => {
    ...
    // iter 3.5: denominator for emission verdicts stays alpha-only so the
    // thresholds (highEmissionShare >= 60 ⇒ aligned_with_emission, etc.)
    // keep their iter-125 meaning. If totalTao expanded to include root,
    // a portfolio that's 60% root + 40% perfectly-aligned alpha would
    // misread as "mixed". Alpha denominator preserves the original signal.
    if (!positionsWithEmission.length || totalAlphaTao <= 0) {
    ...
    const weightedEmissionPct = weightedEmissionNum / totalAlphaTao;
    const highEmissionShare = (highEmissionTao / totalAlphaTao) * 100;
    const zeroEmissionShare = (zeroEmissionTao / totalAlphaTao) * 100;
...
export async function buildReport(coldkey) {
  ...
  // iter 3.5: latestBalance pulled into the initial fan-out so portfolio() can
  // use balance_total for the headline τ. Soft-fails to null on Taostats hiccup
  // — portfolio() falls back to the old alpha-sum behaviour in that case.
  const [taoPrice, holdings, history, screener, usdAud, latestBalance] = await Promise.all([
    getTaoPrice(),
    getHoldings(coldkey),
  ...
  const port = await portfolio({ holdings, screener, taoPrice, usdAud, latestBalance });
  ...
  const currentFyStartYear = now.getUTCMonth() >= 6 ? now.getUTCFullYear() : now.getUTCFullYear() - 1;
  const fysToFetch = [currentFyStartYear - 1, currentFyStartYear];
  // iter 3.5: latestBalance is already fetched in the initial Promise.all
  // (above) and reused here for pnlGroundTruth so we don't burn two API
  // calls on the same endpoint per report build.
