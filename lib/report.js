...
export async function taxYearSection({ coldkey, taoPrice, usdAud }) {
  ...
  const [balanceSeries, ...fyTransferRows] = await Promise.all([
    getBalanceHistory(coldkey, 730).catch(() => []),
    ...fysToFetch.map((sy) =>
      getTaxReportRange(coldkey, `${sy}-07-01`, `${sy + 1}-06-30`).catch(() => []),
    ),
  ]);
  ...
  fysToFetch.forEach((sy, i) => fyRowsBySy.set(sy, fyTransferRows[i]));
