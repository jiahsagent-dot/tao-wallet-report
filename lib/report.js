...
export async function pnlGroundTruth({ coldkey, days, taoPrice, usdAud, rows: preFetchedRows, balance: preFetchedBalance, rowsFetchError: preFetchedRowsError, balanceFetchError: preFetchedBalanceError, forceShadow = false }) {
  ...
  if (shadowFreeApi?.ok) {
    ...
    // iter 205 verify exposed a 5% Mum-wallet drift (substrate 1.7856τ vs Taostats
    // 1.7236τ). iter 262 already proved the FREE leg matches exactly to RAO
    // (299916014 = 0.299916014τ both sources). So the entire drift must live in
    // the stake/alpha leg — almost certainly stale Taostats /coldkey_alpha_shares
    // (lessons_taostats_alpha_shares_stale.md). Surface the split so the UI
    // tooltip + future AI insights can attribute drift correctly.
    const canonicalStakeTao = Number(balance.stakedTao) || 0;
    const canonicalFreeTao = currentPortfolioTao - canonicalStakeTao;
