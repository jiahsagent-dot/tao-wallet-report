...
export async function pnlGroundTruth({ coldkey, days, taoPrice, usdAud, rows: preFetchedRows, balance: preFetchedBalance, rowsFetchError: preFetchedRowsError, balanceFetchError: preFetchedBalanceError, forceShadow = false }) {
  ...
  // null (no badge, no error). Still opt-in via ARCHIVE_STARTING_SHADOW=1 so
  // latency budget on every report isn't blown until stake-valuation parity
  // is also closed. Production critical path stays on Taostats startingBalanceTao.
  const archiveStartingEnabled = process.env.ARCHIVE_STARTING_SHADOW === '1';
  ...
  if (archiveStartingBalance?.ok && Number.isFinite(archiveStartingBalance.totalTao)) {
    ...
    const alignmentSecondsOff = eodMs
      ...
      : null;
    const status = Math.abs(driftTao) < 0.001 ? 'match'
      : Math.abs(driftTao) < 0.1 ? 'drift'
      : 'wide';
