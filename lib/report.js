...
export async function buildReport(coldkey) {
  ...
  // Portfolio-level 24h Δ — picks the snapshot closest to 24h before the
  // latest one and reports the τ delta. Surfaces an emotional "you're up/down
  // today" anchor above §1. Soft-omits if we don't have enough history.
  let delta24h = null;
  if (Array.isArray(balanceSeries) && balanceSeries.length >= 2) {
    const last = balanceSeries[balanceSeries.length - 1];
    const lastT = new Date(last.timestamp).getTime();
    const targetT = lastT - 24 * 60 * 60 * 1000;
    let prior = null;
    ...
    for (let i = balanceSeries.length - 2; i >= 0; i--) {
      ...
      // stop searching once we're walking AWAY from target
      if (t < targetT - 12 * 60 * 60 * 1000) break;
    }
    if (prior && prior.totalTao > 0) {
