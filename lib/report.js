...
export async function pnlGroundTruth({ coldkey, days, taoPrice, usdAud, rows: preFetchedRows, balance: preFetchedBalance }) {
  let [rows, balance] = preFetchedRows && preFetchedBalance !== undefined
  ...
  // If we were handed pre-fetched rows from a wider window, filter to the
  // headline `days` window so the "Over last N days" copy stays honest.
  if (preFetchedRows) {
    const cutoffMs = Date.now() - days * 24 * 60 * 60 * 1000;
    rows = rows.filter((r) => !r.date || new Date(r.date).getTime() >= cutoffMs);
  }
...
export async function drawdownSection({ coldkey, days = 365 }) {
  const series = await getBalanceHistory(coldkey, days).catch(() => []);
  if (series.length < 3) {
...
export async function taxYearSection({ coldkey, taoPrice, usdAud, rows: preFetchedRows }) {
  const rows = preFetchedRows ?? (await getTaxReport(coldkey, 730).catch(() => []));
  if (!rows.length) {
    return { available: false, reason: 'no_tax_data' };
  }
  ...
  for (const r of rows) {
    const t = r.transaction_type;
    if (t === 'transfer_in') {
      transfers.push({ date: r.date, in: Number(r.credit_amount || 0), out: 0 });
    } else if (t === 'transfer_out') {
      transfers.push({ date: r.date, in: 0, out: Number(r.debit_amount || 0) });
    } else if (!t && r.total_balance != null) {
      snapshots.push({ date: r.date, totalBalanceTao: Number(r.total_balance) });
    }
  }
  if (snapshots.length < 2) {
    return { available: false, reason: 'insufficient_snapshots' };
  }
  ...
  const firstSy = fyOf(snapshots[0].date);
  const lastSy = fyOf(snapshots[snapshots.length - 1].date);

  const buckets = [];
  for (let sy = firstSy; sy <= lastSy; sy += 1) {
    const ey = sy + 1;
    const label = `FY${String(sy).slice(-2)}-${String(ey).slice(-2)}`;
    const fyStartMs = new Date(`${sy}-07-01T00:00:00Z`).getTime();
    const fyEndMs = new Date(`${ey}-06-30T23:59:59Z`).getTime();

    let priorSnap = null;
    const inFy = [];
    for (const s of snapshots) {
      const ms = new Date(s.date).getTime();
      if (ms < fyStartMs) priorSnap = s;
      else if (ms <= fyEndMs) inFy.push(s);
    }
    if (inFy.length === 0) continue;

    const startBal = priorSnap ? priorSnap.totalBalanceTao : inFy[0].totalBalanceTao;
    const endBal = inFy[inFy.length - 1].totalBalanceTao;
    let transferIn = 0;
    let transferOut = 0;
    for (const tx of transfers) {
      const ms = new Date(tx.date).getTime();
      if (ms >= fyStartMs && ms <= fyEndMs) {
        transferIn += tx.in;
        transferOut += tx.out;
      }
    }
    ...
    buckets.push({
      ...
      snapshotCount: inFy.length,
      isCurrentFy: sy === lastSy,
...
export async function buildReport(coldkey) {
  ...
  // Single shared tax-report fetch (730d window covers both pnlGroundTruth's
  // 365d view and the AU tax-year split). Avoids duplicate calls hammering
  // the free-tier 60 req/min limit during cold-start.
  const [taxRows, latestBalance] = await Promise.all([
    getTaxReport(coldkey, 730).catch(() => []),
    getLatestBalance(coldkey).catch(() => null),
  ]);
  const [p, gt, dd, ty] = await Promise.all([
    ...
    drawdownSection({ coldkey, days: 365 }).catch(() => ({
      available: false,
      reason: 'fetch_failed',
    })),
    taxYearSection({ coldkey, taoPrice, usdAud, rows: taxRows }).catch(() => ({
      available: false,
      reason: 'fetch_failed',
    })),
  ]);
