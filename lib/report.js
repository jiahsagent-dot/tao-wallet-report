...
// §1 PORTFOLIO
export async function portfolio({ holdings, screener, taoPrice }) {
  const usdAud = await getUsdToAud();
...
// §3 YIELD
export async function yieldSection({ holdings }) {
  // Need APY per (netuid, hotkey). Group lookups by netuid to avoid hammering.
  const netuids = [...new Set(holdings.map((h) => h.netuid))];
  const apyBySubnet = new Map();
  await Promise.all(
    netuids.map(async (nid) => {
      try {
        const apy = await getApyFor(nid, holdings.find((h) => h.netuid === nid)?.hotkey);
        apyBySubnet.set(nid, apy);
      } catch {
        apyBySubnet.set(nid, null);
      }
    })
  );
...
export async function buildReport(coldkey) {
  ...
  const [taoPrice, holdings, history, screener] = await Promise.all([
    getTaoPrice(),
    getHoldings(coldkey),
  ...
  const port = await portfolio({ holdings, screener, taoPrice });
  const p = await pnl({
    holdings,
    history,
    screener,
  ...
  const y = await yieldSection({ holdings });
