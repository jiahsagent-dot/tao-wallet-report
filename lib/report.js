...
export async function buildReport(coldkey) {
  ...
  // Iter 154 — capture getLatestBalance errors instead of swallowing as null.
  ...
  // Iter 158 — chain latestBalanceP after balanceSeriesP settles with a 200ms
  // delay. Both functions hit /api/account/history/v1 (getBalanceHistory
  // paginates, getLatestBalance fetches page 1 limit 1), so firing them as
  // independent parallel Promises collides on the per-IP throttle bucket for
  // that endpoint — retries fire in lockstep, both 429 on every attempt,
  // wider [1500, 4000, 10000] budget consumed serialising on the same bucket.
  // iter 157 verify (2026-06-16T10:08Z) proved this on Mum_mantat: http:200
  // but pnlGroundTruth.error = "Taostats /api/account/history/v1 → 429" with
  // 56.8s elapsed despite the path already being in TAOSTATS_PATH_RETRY_WAITS.
  // Fix mirrors iter 155 Phase B at finer granularity — serialise same-endpoint
  // calls, leave non-Taostats and different-endpoint calls parallel.
  let latestBalanceError = null;
  const latestBalanceP = balanceSeriesP
    .then(() => new Promise((r) => setTimeout(r, 200)))
    .then(() => getLatestBalance(coldkey).catch((e) => {
