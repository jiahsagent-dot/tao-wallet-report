...
// §1 PORTFOLIO
export async function portfolio({ holdings, screener, taoPrice, usdAud, balance }) {
  const positions = aggregateHoldings(holdings)
    // coldkey_alpha_shares returns root stake as a phantom netuid 0 row at
    // alpha-price = 0 (root isn't a dTAO subnet token). Drop it — root is
    // surfaced explicitly via the synthetic ROOT position below.
  ...
  // Synthetic ROOT + WALLET rows so §1 reflects real on-chain holdings.
  // coldkey_alpha_shares is alpha-only; without these a root-heavy wallet
  // (Jai's 5Cnz1juP… is 99% root-staked) reports totalTao ≈ 0 even though
  // balance_total shows ~6.78 τ.
  if (balance) {
    if (balance.rootStakedTao > 0) {
      positions.push({
        netuid: 0,
        name: 'Root-staked TAO',
        alphaHeld: null,
        alphaPriceTao: null,
        taoValue: balance.rootStakedTao,
        usdValue: balance.rootStakedTao * taoPrice,
        emissionPct: null,
        pct1h: null, pct1d: null, pct7d: null, pct1m: null,
        hotkeys: [],
        kind: 'root',
      });
    }
    if (balance.liquidityTao > 0) {
      positions.push({
        netuid: -1,
        name: 'Liquid (free + reserved)',
        alphaHeld: null,
        alphaPriceTao: null,
        taoValue: balance.liquidityTao,
        usdValue: balance.liquidityTao * taoPrice,
        emissionPct: null,
        pct1h: null, pct1d: null, pct7d: null, pct1m: null,
        hotkeys: [],
        kind: 'liquid',
  ...
  // Source totalTao from balance.totalTao (= balance_total / 1e9) when
  // available — that's the on-chain truth. Sum-of-positions drifts ~5-10% as
  // alpha prices move between screener refreshes and silently zeroes out for
  // root-heavy wallets.
  const totalTao = balance?.totalTao != null
    ? balance.totalTao
...
export async function buildReport(coldkey) {
  ...
  const port = await portfolio({ holdings, screener, taoPrice, usdAud, balance: latestBalance });
  ...
  // latestBalance was already awaited above and passed to portfolio().
