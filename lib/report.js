...
export async function pnlGroundTruth({ coldkey, days, taoPrice, usdAud, rows: preFetchedRows, balance: preFetchedBalance }) {
  ...
  // Iter 129: name the reconstruction methodology explicitly so AI Insights
  // can reason about it (rootOnly-symmetric reconstruction, ~170d retention
  // horizon, asymmetric starting-vs-current alpha treatment) instead of being
  // told just the numbers. Path detected from row tagging in
  // getTaxReportRangeFree() (_source: 'free:history/v1@rootOnly:*').
  const isFreePath = rows.some((r) => typeof r?._source === 'string' && r._source.startsWith('free:'));
  const _methodologyNote = (() => {
    const formula = 'profit = current_balance + transfers_out − transfers_in − starting_balance';
    const asymmetry = 'Starting balance excludes alpha-as-tao (tax/v1 quirk); current balance includes it. This asymmetry is preserved for parity with the official Taostats tax report.';
    const window = windowIsShortened
      ? ` Requested ${days}d window truncated to ${effectiveWindowDays}d of actual snapshot coverage (history endpoint retains ~170 days); annualisations should use effectiveWindowDays, not windowDays.`
      : '';
    if (isFreePath) {
      return `Source: free-tier reconstruction from /api/account/history/v1 (rootOnly = balance_free + balance_reserved + balance_staked_root) + /api/transfer/v1. Row shape matches paid /api/accounting/tax/v1 rao-for-rao (iter-105 probe). Formula: ${formula}. ${asymmetry}${window}`;
    }
    return `Source: Taostats paid /api/accounting/tax/v1. Formula: ${formula}. ${asymmetry}${window}`;
  })();
