...
export async function taxYearSection({ coldkey, taoPrice, usdAud }) {
  const rows = await getTaxReport(coldkey, 730).catch(() => []);
  if (!rows.length) {
...
export async function buildReport(coldkey) {
  ...
  const [p, gt, dd, ty] = await Promise.all([
    ...
    taxYearSection({ coldkey, taoPrice, usdAud }).catch(() => ({
      available: false,
      reason: 'fetch_failed',
    })),
  ]);
