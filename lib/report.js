...
export async function pnlGroundTruth({ coldkey, days, taoPrice, usdAud, rows: preFetchedRows, balance: preFetchedBalance, rowsFetchError: preFetchedRowsError }) {
  ...
  if (preFetchedRows !== undefined && preFetchedBalance !== undefined) {
    ...
    balance = await getLatestBalance(coldkey).catch(() => null);
  }

  if (!rows.length || !balance) {
    let reason;
    if (rowsFetchError) reason = 'tax_fetch_failed';
    else if (!rows.length) reason = 'no_tax_data';
    else reason = 'no_balance';
    return {
      available: false,
      reason,
      ...(rowsFetchError ? { error: rowsFetchError } : {}),
    };
  }
