...
// Anonymous-tier rate limit on Pollinations is strict (observed in iter 30:
// 3 back-to-back 429s within 15s when verifying). Cache the cooldown on
// globalThis so subsequent calls during the window short-circuit to the
// next provider instead of re-hitting and amplifying the throttle.
const POLLINATIONS_429_COOLDOWN_MS = 60_000;
const POLLINATIONS_5XX_COOLDOWN_MS = 10_000;

function pollinationsCoolingDown() {
  const until = globalThis.__pollinationsCooldownUntil || 0;
  return Date.now() < until;
}

function setPollinationsCooldown(ms) {
  globalThis.__pollinationsCooldownUntil = Date.now() + ms;
}

// Exposed for /api/health — read-only snapshot of the Pollinations cooldown
// for this Vercel instance. Returns freshly-computed remainingMs so callers
// don't do their own clock math.
export function getPollinationsCooldownState() {
  const until = globalThis.__pollinationsCooldownUntil || 0;
  const now = Date.now();
  const coolingDown = now < until;
  return {
    coolingDown,
    untilMs: until || null,
...
function parseRetryAfter(headerValue, fallbackMs) {
  ...
  if (Number.isFinite(asInt) && asInt > 0) {
    return Math.min(fallbackMs, asInt * 1000);
  }
  return fallbackMs;
}

async function callPollinations({ system, user, maxTokens }) {
  if (pollinationsCoolingDown()) {
    throw new Error('pollinations cooling down (429 recent)');
  }
  ...
  if (res.status === 429) {
    const cool = parseRetryAfter(res.headers.get('retry-after'), POLLINATIONS_429_COOLDOWN_MS);
    setPollinationsCooldown(cool);
    throw new Error(`pollinations 429 (cooldown ${Math.round(cool / 1000)}s)`);
  }
  if (res.status >= 500 && res.status < 600) {
    setPollinationsCooldown(POLLINATIONS_5XX_COOLDOWN_MS);
    throw new Error(`pollinations ${res.status} (cooldown 10s)`);
  }
