...
export const ENTRIES = [
  {
    date: '2026-05-27',
    tag: 'infra',
    title: 'Free-PnL silent-failure instrumentation: getTaxReportRangeFree() now logs one warning line whenever it emits empty/partial output (history walk empty, snapshots collapsed to zero, or merged result empty) so the runtime logs finally explain why §2 ships "no_tax_data" — five iters chasing this without visibility was too many',
    body:
      'iter 118 promise-dedup shipped READY, but visual verify on the preview still surfaced §2 = no_tax_data for mantat (the lightweight coldkey — root + subnets both 500\'d on unrelated /api/price/latest/v1 + /api/delegation/v1 races at the top-level Promise.all). Reread runtime logs across iters 114-118: only taoGet\'s retry warnings come through ("[taostats] /api/account/history/v1 → 429"), not anything from getTaxReportRangeFree itself. The function has at least three silent-failure modes — getHistoryRowsCached page-1 throw caught upstream, historyRowsInWindow filter producing zero rows even when allRows is non-empty (window-boundary edge case), or out array empty after dedup — and none of them log anything before propagating up through pnlGroundTruth\'s .catch(() => []). Without runtime visibility into WHICH leg is failing, every iter is a guess. iter 119 adds three console.warn lines: (a) allRows empty (page-1 history fetch failed or coldkey has no history), (b) out empty but allRows non-empty (window/filter edge case), (c) merged empty despite out being non-empty (defensive — shouldn\'t happen). One-line each, prefix [free-pnl] for easy grep. Zero behaviour change. Diagnostic-only iter — next fire reads the new warnings on the live preview and knows exactly what to fix.',
