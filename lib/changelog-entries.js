...
export const ENTRIES = [
  {
    date: '2026-05-28',
    tag: 'infra',
    title: 'Free-PnL reconstruction switches from rootOnly to balance_total snapshot walk (DRAFT, awaiting signoff): lib/taostats.js getTaxReportRangeFree now derives per-day snapshot balance from balance_total (root + alpha-as-tao) instead of rootOnly (balance_free + balance_reserved + balance_staked_root). iter 139 balance_delta_probe verdict for subnets coldkey block 7534068 (2026-02-13 mid-window 2.556τ inbound — the REAL bug, not the 2025-12-06 boundary divergent-by-retention rows) was total_delta_matches_only: rootΔ off by 0.7τ, totalΔ matched within 0.024τ. The credit lands in alpha, not root, so rootOnly reconstruction structurally misses stake-movement-routed inbound events that paid\'s tax/v1 engine flattens into transfers_in. Trade-off: free PnL semantics shift from "TAO root in/out" to "portfolio-value delta" — alpha movements count as part of the balance walk, same-day transfer-row matches still net out via transferNetByDate. This matches paid\'s tax/v1 numerically. SHIPPED ON A DRAFT BRANCH (iter140-balance-total-walk-draft) AWAITING JAI SIGNOFF — not merged to free-pnl-preview-verify',
    body:
      'iter 139 balance_delta_probe live verdict resolved the iter-138 fix-path fork: for block 7534068 (subnets coldkey 5EKFph3D839fxdbQwhAHyM4CQzBHNpLSecUAteNZKqW1G5cd, 2026-02-13 +2.556τ in, deep mid-window) rootΔ=+1.855τ (off by 0.7τ vs expected 2.556τ) but balance_total Δ=+2.580τ (matched within 0.024τ tolerance). Verdict: total_delta_matches_only. The two 2025-12-06 boundary blocks (7036937, 7036948) verdict as pre_history_retention as predicted — they\'re the iter-134 divergent-by-retention semantic where free is honestly computing PnL-since-first-snapshot. So the REAL bug is a single structural miss: a stake-movement event (likely a remove_stake or alpha→root unstake) credits alpha-as-tao, not root, and the rootOnly reconstruction never sees it. Path (ii) is the cheapest fix that catches all such events — switch the per-day snapshot walk in getTaxReportRangeFree from rootOnly to balanceTotal, which DOES capture alpha-as-tao deltas. The diff is small: add a balanceTotal helper alongside the existing rootOnly (kept in place for the iter-139 debug probe), swap calls in the day-row emission loop, rename prevRoot → prevBalance, update _source tags from @rootOnly to @balanceTotal. Daily_income still subtracts transferNetByDate so honest TAO-route transfers stay neutral; alpha-routed credits naturally show as "income" alongside subnet emission yield, which is exactly what paid does. PnL semantics shift, though: "rootOnly-since-snapshot" → "portfolio-value-delta-since-snapshot, netted against transfer/v1 transfers". The numerical output matches paid; the conceptual definition is what reviewers see in the §0 AI Insights and §1 narrative if they\'re reading carefully. Three options on the table for Jai: (1) ship as-is — pure parity with paid, alpha movements count as income; (2) ship with a _methodologyNote on AI Insights that documents the new semantics so the LLM and any human reading the report knows what "transfers" + "income" mean under free; (3) reject path (ii) and pursue path (iii) — add /api/dtao/stake_balance_change/v1 or similar as a second source so rootOnly can stay as the PnL definition. Path (iii) is more work, preserves the TAO-route semantics, but might still miss alpha-route credits depending on the staking endpoint\'s scope. This iter ships path (ii) ON A DRAFT BRANCH so Jai has a concrete diff + PR body to review the semantics shift against — not merged into free-pnl-preview-verify until he picks (1), (2), or (3) explicitly.',
