...
export const ENTRIES = [
  {
    ...
    body:
      'Australian users want the same answer their accountant wants: how much TAO did this wallet earn in FY24-25, and what is the FY25-26 number tracking at right now? §2 PnL gains a tax-year breakdown table below the drawdown panel: one row per AU FY (Jul 1 → Jun 30) showing FY label, window, start balance, end balance, transfers in, transfers out, PnL in τ, return %, and PnL in A$. Same formula as the headline ground-truth number (end + transfer_out − transfer_in − start) applied per-FY with the carry-in balance taken from the last snapshot before each FY starts so balances pre-dating the window aren\'t double-counted as profit. The current FY is highlighted with an "(in progress)" tag and a subtle row tint. Pulls 730 days of tax-report rows in parallel with the existing pnlGroundTruth call — soft-omits cleanly if the wider window fails. Zero impact on wallets newer than two years (you just get one FY row).',
