...
// iter 144: per-endpoint timeout override map. The 15s blanket from iter 143
// was the right safety floor against the 2026-06-15 multi-hour outage but it's
// suboptimal in both directions for normal operation:
//   - Lightweight endpoints (/api/price/latest/v1, /api/account/latest/v1)
//     respond in <1s on a healthy Pro plan. A 15s wait when they hang adds
//     ~14s of useless dead time to the report build — better to fail-fast in
//     5–8s so the route-level error envelope or .catch() degradation fires
//     sooner, leaving more of the 60s maxDuration budget for the rest of the
//     parallel fanout.
//   - Heavy paginated/aggregating endpoints (/api/accounting/tax/v1 returns
//     a full FY of tax events server-side, /api/account/history/v1 with
//     limit=200 page=N can take 8–12s on cold cache, /api/delegation/v1
//     same shape) can legitimately need >15s on a healthy but loaded Taostats
//     and would otherwise false-positive timeout on a wallet that just needs
//     a few more seconds. 20–25s gives genuine headroom without burning the
//     full maxDuration on a stall.
...
const TAOSTATS_PATH_TIMEOUTS = {
  '/api/price/latest/v1': 5000,
  '/api/account/latest/v1': 8000,
  ...
  '/api/dtao/validator/yield/latest/v1': 12000,
};
