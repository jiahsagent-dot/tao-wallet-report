...
export async function getTaxReportRangeFree(coldkey, start, end) {
  ...
  // 2b. Move-stake events (iter 139). The substrate `transfer_stake` /
  // `move_stake` extrinsics move staked TAO directly between coldkeys without
  // emitting a `balances.Transfer` event, so transfer/v1 misses them entirely.
  // For Jai's Root coldkey this undercounted transfer_out by 6.03 τ over 365d
  // (4 events, all moving root-staked TAO to his other coldkeys), which flipped
  // the headline PnL from the true +0.20 τ (paid tax/v1 + spreadsheet) to a
  // bogus −5.83 τ — physically impossible on a root-staked wallet.
  //
  // delegation/v1 exposes both sides of each move via `is_transfer: true`:
  //   - UNDELEGATE row, nominator=X → X is the SOURCE (lost TAO).
  //                                    transfer_address = destination.
  //   - DELEGATE row,   nominator=X → X is the DESTINATION (gained TAO).
  //                                    transfer_address = source.
  // One paginated query (nominator=X) finds both directions; split by action.
  // Clip to [effectiveStartMs, endD] so it composes with the snapshot window.
  let dPage = 1;
  while (dPage <= 50) {
    const j = await taoGet('/api/delegation/v1', {
      nominator: coldkey,
    ...
    if (rows.length === 0) break;
    for (const r of rows) {
      if (r.is_transfer !== true) continue;
      const ts = r.timestamp;
      if (!ts) continue;
      const tsMs = new Date(ts).getTime();
      if (tsMs < effectiveStartMs || tsMs > endD.getTime()) continue;
      const taoAmt = Number(r.amount || 0) / RAO;
      if (r.action === 'UNDELEGATE') {
        transferRows.push({
          transaction_type: 'transfer_out',
          debit_amount: taoAmt,
          date: dateStr(ts),
          timestamp: ts,
          _source: 'free:delegation/v1@move_stake_out',
        });
      } else if (r.action === 'DELEGATE') {
        transferRows.push({
          transaction_type: 'transfer_in',
          credit_amount: taoAmt,
          date: dateStr(ts),
          timestamp: ts,
          _source: 'free:delegation/v1@move_stake_in',
        });
      }
    }
    if (rows.length < 200) break;
...
// Latest balance breakdown for a coldkey from /api/account/history/v1 (most
// recent snapshot row). Returns the full per-bucket split so callers can
// distinguish root-staked TAO (~9% APR) from alpha-staked-as-tao from idle
// (free + reserved) — needed by portfolio.totalTao, which must source from
// balance_total to avoid understating root-heavy wallets to ~0 (the bug
// behind Jai's "portfolio shows 0" complaint).
export async function getLatestBalance(coldkey) {
  ...
  if (!rec) return null;
  const RAO = 1e9;
  const free = Number(rec.balance_free || 0) / RAO;
  const reserved = Number(rec.balance_reserved || 0) / RAO;
  const stakedRoot = Number(rec.balance_staked_root || 0) / RAO;
  const stakedAlphaAsTao = Number(rec.balance_staked_alpha_as_tao || 0) / RAO;
  return {
    stakedTao: Number(rec.balance_staked || 0) / RAO,
    totalTao: Number(rec.balance_total || 0) / RAO,
    freeTao: free,
    reservedTao: reserved,
    rootStakedTao: stakedRoot,
    alphaStakedAsTao: stakedAlphaAsTao,
    liquidityTao: free + reserved,
