...
async function taoGet(path, params = {}) {
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    const timer = setTimeout(() => controller.abort(), TAOSTATS_DEFAULT_TIMEOUT_MS);
    let r;
    try {
      ...
      if (isAbort) {
        console.warn(`[taostats] ${path} → timeout after ${TAOSTATS_DEFAULT_TIMEOUT_MS}ms (fail-fast, no retry)`);
        throw new Error(`Taostats ${path} → timeout after ${TAOSTATS_DEFAULT_TIMEOUT_MS}ms`);
      }
