...
export async function getLatestBalance(coldkey) {
  ...
  return {
    ...
    rootStakedTao: Number(rec.balance_staked_root || 0) / RAO,
    alphaStakedAsTao: Number(rec.balance_staked_alpha_as_tao || 0) / RAO,
    freeTao: Number(rec.balance_free || 0) / RAO,
    reservedTao: Number(rec.balance_reserved || 0) / RAO,
    liquidityTao: Number(rec.balance_liquidity || 0) / RAO,
