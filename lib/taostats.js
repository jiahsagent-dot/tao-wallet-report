...
//
// NB iter 108 (reverted iter 107): the free path is back to OPT-IN. iter 107
// flipped the default to free and live-tested against the demo coldkeys; the
// subnets coldkey (5EKFph…) is YOUNGER than the prod 365d window so
// getTaxReportRangeFree could not find a snapshot at-or-before start, emitted
// no snapshot row, and §2 PnL went to "no_balance_snapshots". Fix-forward:
// teach getTaxReportRangeFree to fall back to the OLDEST available history
// snapshot when none exists ≤ start (and only include transfers AFTER that
// fallback date so the formula stays consistent). Until then: paid is default.
...
export async function getTaxReportRangeFree(coldkey, start, end) {
  ...
  //    Same algorithm as the iter-105 probe.
  ...
  if (startingRow) {
    ...
    out.push({
      ...
      _source: 'free:history/v1@rootOnly',
    });
  }

  // 2. Transfer rows in [start, end].
  page = 1;
  const transferRows = [];
  while (page <= 20) {
    const j = await taoGet('/api/transfer/v1', {
      address: coldkey,
      timestamp_start: Math.floor(startD.getTime() / 1000),
