...
export async function getTaxReportRangeFree(coldkey, start, end) {
  ...
  let newestSeenRow = null; // iter 111: needed for closing snapshot so
                             // lastSnapshotDate diverges from firstSnapshotDate
                             // and iter 110's effectiveWindowDays actually fires.
  let page = 1;
  outer: while (page <= 20) {
  ...
  if (startingRow) {
    out.push({
      ...
      total_balance: rootOnly(startingRow),
  ...
  // Iter 111: closing snapshot at the most recent history row so
  // pnlGroundTruth's lastSnapshotDate actually reflects end-of-window. Without
  // this, lastSnapshotDate === firstSnapshotDate under free mode and iter 110's
  // effectiveWindowDays ternary falls through to `days` — i.e. a "365d" badge
  // sitting on top of a ~172d reconstruction, which is precisely the lie iter
  // 110 was meant to prevent. The row's total_balance is read by pnlGroundTruth
  // into lastSnapshotBalance (assigned, never returned, never used in PnL math)
  // so the basis (rootOnly here for consistency with the starting row) does not
  // affect any downstream number. Only emit if it's strictly newer than the
  // starting row, to avoid degenerate single-row windows producing a 1-day
  // effectiveWindowDays.
  if (
    newestSeenRow &&
    startingRow &&
    new Date(newestSeenRow.timestamp).getTime() >
      new Date(startingRow.timestamp).getTime()
  ) {
    out.push({
      transaction_type: null,
      total_balance: rootOnly(newestSeenRow),
      date: dateStr(newestSeenRow.timestamp),
      timestamp: newestSeenRow.timestamp,
      _source: 'free:history/v1@rootOnly:closing',
    });
  }
