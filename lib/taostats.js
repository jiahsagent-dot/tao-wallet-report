...
// iter 177 — CoinGecko free-API fallback for getTaoPrice. Cold-start Vercel
// lambdas have priceCache.value===null, so the iter 146 stale-OK fallback
// can't help: a single Taostats /api/price/latest/v1 429 on a fresh instance
// throws and the entire buildReport returns http:500 (the exact failure
// pattern iter 173/176 saw on root/mum_root/mum_mantat — wall_ms 6–17s,
// nowhere near the 60s pnlGroundTruth ceiling). CoinGecko's
// /api/v3/simple/price endpoint is anon-rate-limited but reachable from
// Vercel (per lessons_vercel_binance_block.md; Binance is the blocked host,
// not CoinGecko). 4s timeout to keep total getTaoPrice latency bounded.
const COINGECKO_TAO_URL = 'https://api.coingecko.com/api/v3/simple/price?ids=bittensor&vs_currencies=usd';
const COINGECKO_TIMEOUT_MS = 4000;

async function getTaoPriceCoinGecko() {
  const ctl = new AbortController();
  const timer = setTimeout(() => ctl.abort(), COINGECKO_TIMEOUT_MS);
  try {
    const resp = await fetch(COINGECKO_TAO_URL, { signal: ctl.signal, headers: { accept: 'application/json' } });
    if (!resp.ok) throw new Error(`coingecko http:${resp.status}`);
    const j = await resp.json();
    const price = Number(j?.bittensor?.usd);
    if (!Number.isFinite(price) || price <= 0) throw new Error('coingecko: missing/invalid price');
    return price;
  } finally {
    clearTimeout(timer);
  }
...
// Live TAO/USD price (cached 60s fresh, 5min stale-while-error, then CoinGecko fallback).
export async function getTaoPrice() {
  ...
  try {
    ...
    // iter 177 — CoinGecko free-API fallback for cold-start lambdas with no warm cache.
    try {
      const price = await getTaoPriceCoinGecko();
      priceCache.value = price;
      priceCache.fetchedAt = now;
      console.warn(`[taostats] price fetch failed (${e.message}) — served CoinGecko fallback $${price}`);
      return price;
    } catch (e2) {
      console.warn(`[taostats] price fetch failed (${e.message}); CoinGecko fallback also failed (${e2.message})`);
      throw e;
    }
  }
}
...
//
// Iter 161: cumulative pagination budget cap with partial-data fallback.
// Mantat (5CTRC3sQ…ArLn, 12.28τ multi-page chain) iter 142 verify ran at
// 504/60.8s — even with iter 157's 200ms inter-page delay + iter 156's
// [1500, 4000, 10000] retry budget, a later-page 429 burned full retries
// while earlier pages had already eaten ~40-50s, compounding past Vercel's
// 60s ceiling → FUNCTION_INVOCATION_TIMEOUT with no JSON body, no verdict.
// The budget cap (30s elapsed since this call started) keeps the rest of
// buildReport ~25s of headroom under 60s; mid-pagination throws return
// partial rows instead of bubbling so the verdict path survives. Page-1
// throws still bubble (preserves iter 154 lying-catch fix — we don't want
// to mask a total fetch failure as an empty-array success).
const DELEGATION_HISTORY_BUDGET_MS = 30000;
...
export async function getDelegationHistory(coldkey) {
  ...
  while (true) {
    if (Date.now() - startedAt > DELEGATION_HISTORY_BUDGET_MS) {
      console.warn(
        `[getDelegationHistory] budget exceeded for ${coldkey} at page ${page} after ${out.length} rows — returning partial data`,
      );
      truncated = true;
      truncationReason = 'budget_exceeded';
      break;
    }
    let j;
    try {
      ...
      // Mid-pagination throw (page > 1): we already have rows from earlier
      // pages, so return partial instead of collapsing the whole verdict
      // path. Page-1 throw bubbles up unchanged (preserves iter 154 fix).
      if (page === 1) throw err;
      console.warn(
        `[getDelegationHistory] page ${page} threw for ${coldkey} after ${out.length} rows — returning partial data: ${err?.message}`,
      );
      truncated = true;
      truncationReason = 'page_fetch_failed';
      break;
    }
    const rows = j?.data || [];
    for (const r of rows) {
    ...
    page += 1;
    if (page > 50) break;
    // Iter 157: 200ms inter-page delay so each subsequent /api/delegation/v1
    // page fetch gets the shared Vercel egress IP to itself for the duration
    // of its retry chain. Mirror of iter 155 Phase B serialisation at finer
    // granularity. iter 156 widened delegation/v1 retries to [1500, 4000,
    // 10000] but Mantat (12.28τ, multi-page chain) still 429'd at 18.7s
    // because the next-page fetch hit the throttle bucket mid-recovery.
    // Per-page max overhead at p50 = 200ms × ~5 pages = 1s; bounded.
    await new Promise((resolve) => setTimeout(resolve, 200));
  }
  if (truncated) {
    Object.defineProperty(out, 'partial', { value: true, enumerable: false });
    Object.defineProperty(out, 'partialReason', { value: truncationReason, enumerable: false });
    Object.defineProperty(out, 'pagesFetched', { value: page, enumerable: false });
  }
  return out;
}
...
export async function getTaxReportRangeFree(coldkey, start, end) {
  ...
  for (let i = 0; i < snapshotsAsc.length; i++) {
    ...
    const row = {
      ...
      timestamp: r.timestamp,
      // iter 228 — forward Taostats /api/account/history/v1.block_number onto
      // each snapshot row so the archive-starting-balance shadow can anchor
      // its sample to the EXACT chain block Taostats used for balance_staked,
      // not a 12s/block extrapolation that drifted ~28h over a 30d window.
      block_number: r.block_number != null ? Number(r.block_number) : null,
      _source:
        i === 0
          ? (usedFallback
...
// Iter 265 — Priority #1 shadow telemetry (Option Y). Unwindowed
// /api/transfer/v1 aggregator: pages the FULL transfer history for a
// coldkey (no timestamp clip), sums credit_amount → transferInTao and
// debit_amount → transferOutTao. Iter 263 proved this captures the
// wallet-birth seed transfer (Mum_mantat GREEN, 0 mτ vs paid) but MISSES
// stake-unlock rows (paid tax/v1 rows with from_address=null; blocks
// 7534061/7534068 dtao reshuffle and 7052826/7052858 subnet-reg refund),
// and iter 264 ruled out separability of those 4 rows in free
// /api/delegation/v1. Consequence: this helper is SHADOW-ONLY for iter 265,
// wired into pnlGroundTruth as observability telemetry (shadowFreeTaxV2)
// but never promoted to canonical. Enables future drift-monitoring of the
// two known coverage gaps (birth transfers, stake unlocks) without touching
// user-visible PnL.
export async function getTransferAggregateFree(coldkey) {
  const RAO = 1e9;
  let page = 1;
  let transferInTao = 0;
  ...
  while (page <= 20) {
    ...
    for (const t of rows) {
      ...
      const to = t.to?.ss58 || t.to;
      const from = t.from?.ss58 || t.from;
      if (to === coldkey) transferInTao += amountTao;
      else if (from === coldkey) transferOutTao += amountTao;
      transferCount += 1;
    }
    if (rows.length < 200) break;
    page += 1;
  }
  return { transferInTao, transferOutTao, transferCount };
}
...
//
// iter 160: wrapped in a stale-OK in-memory cache mirroring iter 146's price
// cache. Key = `${coldkey}_${days}`. 60s fresh, 5min stale-OK. When
// /api/account/history/v1 returns 429 (or any error) and a recent successful
// result is cached, we serve the stale series so the verdict path survives
// the throttle instead of failing pnlGroundTruth to balance_fetch_failed.
// Per lessons_taostats_account_endpoints.md, /history/v1 already lags 24h
// (daily snapshots), so a 5min-stale snapshot is functionally identical
// freshness to a fresh fetch — high cacheability.
const BALANCE_HISTORY_FRESH_MS = 60_000;
const BALANCE_HISTORY_STALE_OK_MS = 5 * 60_000;
const balanceHistoryCache = new Map();

export function _resetBalanceHistoryCache() {
  balanceHistoryCache.clear();
}
export function _getBalanceHistoryCacheState() {
  return { size: balanceHistoryCache.size, keys: [...balanceHistoryCache.keys()] };
}

async function fetchBalanceHistory(coldkey, days) {
  ...
  while (true) {
    ...
    for (const r of rows) {
      ...
      out.push({
        ...
        // iter 209 — also carry free + reserved so callers that use the last
        // history row as a substitute for getLatestBalance (see report.js
        // ~line 2516) have parity. iter 208 patched getLatestBalance but
        // missed this twin path → latestBalance.freeTao came back undefined →
        // canonicalFreeTao = Number(undefined) || 0 = 0, fabricating a
        // -0.299916 τ freeDriftTao on Mum.
        freeTao: Number(r.balance_free || 0) / RAO,
        reservedTao: Number(r.balance_reserved || 0) / RAO,
      });
    }
...
export async function getBalanceHistory(coldkey, days = 365) {
  const cacheKey = `${coldkey}_${days}`;
  const now = Date.now();
  const cached = balanceHistoryCache.get(cacheKey);
  if (cached && now - cached.fetchedAt < BALANCE_HISTORY_FRESH_MS) {
    return cached.value;
  }
  try {
    const value = await fetchBalanceHistory(coldkey, days);
    balanceHistoryCache.set(cacheKey, { value, fetchedAt: now });
    return value;
  } catch (e) {
    if (cached && now - cached.fetchedAt < BALANCE_HISTORY_STALE_OK_MS) {
      const ageS = Math.round((now - cached.fetchedAt) / 1000);
      console.warn(`[taostats] balance-history fetch failed (${e.message}) — serving stale cached ${cached.value.length} rows (${ageS}s old) for ${coldkey}`);
      return cached.value;
    }
    throw e;
  }
...
export async function getLatestBalance(coldkey) {
  ...
  return {
    ...
    // iter 208 — surface free + reserved directly. Prior path derived
    // canonicalFreeTao as (totalTao - stakedTao), which silently included
    // balance_reserved and fabricated a ~0.093τ fake "free-leg drift" on
    // every wallet with a reserved component (iter 268 NOTES). Now report.js
    // can read balance_free + balance_reserved without re-deriving.
    freeTao: Number(rec.balance_free || 0) / RAO,
    reservedTao: Number(rec.balance_reserved || 0) / RAO,
...
// Screener fallback via Taostats /api/dtao/pool/latest/v1. 2026-07-11: tao.app
// invalidated TAOAPP_API_KEY server-side (403 "Invalid API key" from both
// Vercel and the VPS), and the bare getSubnetScreener() throw hard-500'd every
// report. The pool endpoint carries full parity for every screener field the
// report consumes except emission_pct (all consumers null-guard it):
//   name → subnet_name, price_change_1_day → price_1d_pct_change,
//   price_change_1_week → price_7d_pct_change, etc.
// tao_volume_24_hr is in RAO → /1e9 to match the screener's TAO units.
export async function getScreenerFromTaostatsPool() {
  const rows = [];
  let page = 1;
  while (true) {
    const j = await taoGet('/api/dtao/pool/latest/v1', { page, limit: 200 });
    for (const r of j?.data || []) {
      rows.push({
        ...
        price_1h_pct_change: r.price_change_1_hour != null ? Number(r.price_change_1_hour) : null,
        price_1d_pct_change: r.price_change_1_day != null ? Number(r.price_change_1_day) : null,
        price_7d_pct_change: r.price_change_1_week != null ? Number(r.price_change_1_week) : null,
        price_1m_pct_change: r.price_change_1_month != null ? Number(r.price_change_1_month) : null,
        total_volume_tao_1d: r.tao_volume_24_hr != null ? Number(r.tao_volume_24_hr) / 1e9 : null,
      });
    }
    if (!j?.pagination?.next_page || page >= 3) break;
  ...
  return { rows, byNetuid, source: 'taostats-pool-fallback' };
}
