...
// each call previously re-walked /api/account/history/v1 from page 1. Under
// FREE_PNL=1 with the standard plan key, those competing fetches (plus the
// parallel getBalanceHistory + getLatestBalance calls on the same coldkey)
// burst past Vercel's shared-outbound-IP rate ceiling, the two retries trip,
// and getTaxReportRangeFree throws — pnlGroundTruth then catches into an
// empty array and §2 ships "no_tax_data" (iter 113 preview bug). The memo
// keeps history rows for 60s keyed by coldkey so the second FY call is free.
// TTL keeps warm lambdas from serving stale data across distinct user
// requests.
const _freeHistoryMemo = new Map(); // coldkey → { rows, ts }
const FREE_HISTORY_TTL_MS = 60_000;
async function fetchFreeHistoryPagesCached(coldkey) {
  const cached = _freeHistoryMemo.get(coldkey);
  if (cached && Date.now() - cached.ts < FREE_HISTORY_TTL_MS) {
    return cached.rows;
  }
  const allRows = [];
  let page = 1;
  ...
  _freeHistoryMemo.set(coldkey, { rows: allRows, ts: Date.now() });
  return allRows;
}
...
export async function getTaxReportRangeFree(coldkey, start, end) {
  ...
  //    fetchFreeHistoryPagesCached (iter 114). Rows are timestamp-DESC, so we
  //    scan forward and pick the first row at-or-before `start`. The OLDEST
  //    row seen is the fallback for young coldkeys whose history doesn't reach
  //    back to `start` (iter 109 regression fix).
  const allRows = await fetchFreeHistoryPagesCached(coldkey);
