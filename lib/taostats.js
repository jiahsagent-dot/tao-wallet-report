...
//
// As of iter 107 the FREE-tier reconstruction (getTaxReportRangeFree below) is
// the DEFAULT path — iter 106 validated it to Δ=0.000000τ against paid
// /api/accounting/tax/v1 on all three of Jai's coldkeys over 30d windows.
// PAID_PNL=1 forces the legacy paid endpoint (kept as an escape hatch for
// debugging / regression checks); without that flag, we never call paid tax/v1.
export async function getTaxReportRange(coldkey, start, end) {
  if (process.env.PAID_PNL !== '1') {
