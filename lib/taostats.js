...
// iter 128: 3-layer cache (memo → DB → fresh walk) matching iter 125 (history)
// and iter 127 (transfers). Third and last endpoint in the burst-429 triplet
// closed onto the cache shape. After this, FREE_PNL=1 prod flip is structurally
// derisked: cold lambdas serving repeated coldkey requests inside the 15-min
// TTL no longer re-walk any of the three rate-limit-prone endpoints.
// In-flight dedup means parallel calls (e.g. buildReport's top-level
// Promise.all) share one walk.
// Flattened-row shape preserved so downstream callers (pnl(), realised PnL,
// staking sections) don't change.
const _delegationRowsMemo = new Map(); // coldkey → { rows, ts }
const _delegationRowsInFlight = new Map(); // coldkey → Promise<rows>
const _delegationRowsLastSource = new Map(); // coldkey → 'memo'|'db'|'fetch'|'fetch-empty'
const DELEGATION_ROWS_TTL_MS = 60_000;
const DELEGATION_DB_CACHE_TTL_MS = 15 * 60 * 1000;
export function _getDelegationRowsLastSource(coldkey) {
  return _delegationRowsLastSource.get(coldkey) || null;
}
export async function getDelegationHistory(coldkey) {
  const cached = _delegationRowsMemo.get(coldkey);
  if (cached && Date.now() - cached.ts < DELEGATION_ROWS_TTL_MS) {
    _delegationRowsLastSource.set(coldkey, 'memo');
    return cached.rows;
  }
  const dbHit = await (await import('./supabase.js')).delegationCacheRead(coldkey, DELEGATION_DB_CACHE_TTL_MS);
  if (dbHit && Array.isArray(dbHit.rows)) {
    _delegationRowsMemo.set(coldkey, { rows: dbHit.rows, ts: Date.now() });
    _delegationRowsLastSource.set(coldkey, 'db');
    return dbHit.rows;
  }
  const inflight = _delegationRowsInFlight.get(coldkey);
  ...
  const p = (async () => {
    ...
    while (page <= 50) {
      ...
      try {
        ...
        if (page === 1) {
          _delegationRowsLastSource.set(coldkey, 'fetch-empty');
          console.warn(`[taostats] /api/delegation/v1 page 1 failed (${e.message}); returning empty rows (next caller will retry)`);
          return [];
        }
        console.warn(`[taostats] /api/delegation/v1 page ${page} failed (${e.message}); returning partial ${rows.length} delegation rows`);
        break;
      }
      const pageRows = j?.data || [];
    ...
    _delegationRowsMemo.set(coldkey, { rows, ts: Date.now() });
    _delegationRowsLastSource.set(coldkey, 'fetch');
    if (rows.length > 0) {
      (async () => {
        const { delegationCacheWrite } = await import('./supabase.js');
        const ok = await delegationCacheWrite(coldkey, rows);
        if (!ok) console.warn(`[taostats] delegation-rows db cache write failed for ${coldkey.slice(0, 8)}…`);
      })();
    }
    return rows;
  })();
  _delegationRowsInFlight.set(coldkey, p);
  try {
    return await p;
  } finally {
    _delegationRowsInFlight.delete(coldkey);
  }
}
...
// iter 132: extracted from getTaxReportRange so the parity probe can call
// the paid path explicitly regardless of FREE_PNL env state. Production code
// continues to go through getTaxReportRange (env-dispatched) so behaviour
// is unchanged.
export async function getTaxReportRangePaid(coldkey, start, end) {
...
export async function getTaxReportRange(coldkey, start, end) {
  ...
  return getTaxReportRangePaid(coldkey, start, end);
}
    ...
    while (page <= 20) {
      ...
      try {
        ...
        if (page === 1) {
          ...
          // iter 126: tag last_source so the debug endpoint reflects the
          // empty-walk fail instead of a stale 'memo'/'db' from a prior call.
          _historyRowsLastSource.set(coldkey, 'fetch-empty');
...
// iter 127: cross-request cache for /api/transfer/v1 walks per coldkey. Same
// 3-layer hierarchy as getHistoryRowsCached (memo → DB → fresh walk) with the
// same in-flight dedup. Fetched all-time (no timestamp filter), so one cached
// row serves both FY24 and FY25 reconstructions plus the cross-request reuse
// across cold lambdas inside the TTL. Consumer in getTaxReportRangeFree filters
// in-memory by [effectiveStartMs, endD].
const _transferRowsMemo = new Map(); // coldkey → { rows, ts }
const _transferRowsInFlight = new Map(); // coldkey → Promise<rows>
const _transferRowsLastSource = new Map(); // coldkey → 'memo'|'db'|'fetch'|'fetch-empty'
const TRANSFER_ROWS_TTL_MS = 60_000;
const TRANSFER_DB_CACHE_TTL_MS = 15 * 60 * 1000;
export function _getTransferRowsLastSource(coldkey) {
  return _transferRowsLastSource.get(coldkey) || null;
}
async function getTransferRowsCached(coldkey) {
  const cached = _transferRowsMemo.get(coldkey);
  if (cached && Date.now() - cached.ts < TRANSFER_ROWS_TTL_MS) {
    _transferRowsLastSource.set(coldkey, 'memo');
    return cached.rows;
  }
  const dbHit = await (await import('./supabase.js')).transfersCacheRead(coldkey, TRANSFER_DB_CACHE_TTL_MS);
  if (dbHit && Array.isArray(dbHit.rows)) {
    _transferRowsMemo.set(coldkey, { rows: dbHit.rows, ts: Date.now() });
    _transferRowsLastSource.set(coldkey, 'db');
    return dbHit.rows;
  }
  const inflight = _transferRowsInFlight.get(coldkey);
  if (inflight) return inflight;
  const p = (async () => {
    const rows = [];
    let page = 1;
    while (page <= 20) {
      ...
      try {
        ...
        if (page === 1) {
          _transferRowsLastSource.set(coldkey, 'fetch-empty');
          console.warn(`[taostats] transfer-rows page 1 failed (${e.message}); returning empty rows (next caller will retry)`);
          return [];
        }
        console.warn(`[taostats] transfer-rows page ${page} failed (${e.message}); returning partial ${rows.length} transfer rows`);
        break;
      }
      const pageRows = j?.data || [];
    ...
    _transferRowsMemo.set(coldkey, { rows, ts: Date.now() });
    _transferRowsLastSource.set(coldkey, 'fetch');
    // Persist non-empty walks only — empty could be transient burst-429.
    if (rows.length > 0) {
      (async () => {
        const { transfersCacheWrite } = await import('./supabase.js');
        const ok = await transfersCacheWrite(coldkey, rows);
        if (!ok) console.warn(`[taostats] transfer-rows db cache write failed for ${coldkey.slice(0, 8)}…`);
      })();
    }
    return rows;
  })();
  _transferRowsInFlight.set(coldkey, p);
  try {
    return await p;
  } finally {
    _transferRowsInFlight.delete(coldkey);
  }
}
...
export async function getTaxReportRangeFree(coldkey, start, end) {
  ...
  // iter 140: balanceTotal includes alpha-as-tao. Switched the per-day snapshot
  // walk from rootOnly to balanceTotal because iter 139's balance_delta_probe
  // verdict for block 7534068 (subnets coldkey, 2026-02-13 mid-window 2.556τ
  // inbound) was `total_delta_matches_only` — rootΔ off by 0.7τ, totalΔ
  // matched within 0.024τ. The credit lands in alpha, not in root, so a pure
  // rootOnly reconstruction structurally misses these stake-movement-routed
  // inbound events. Trade-off: free PnL semantics shift from "TAO root in/out"
  // to "portfolio-value delta" — alpha movements now count as part of the
  // balance walk (and any same-day matching transfer row will be subtracted
  // out by transferNetByDate, so honest TAO-route transfers stay neutral).
  // This matches what paid's tax/v1 engine produces.
  const balanceTotal = (row) => Number(row.balance_total || 0) / RAO;
  ...
  // iter 127: getTransferRowsCached fetches the all-time transfer list per
  // coldkey through the memo → DB → fresh-walk cache hierarchy (mirrors
  // history-rows cache from iter 125). Filtering by [effectiveStartMs, endD]
  // moves in-memory, so FY24 + FY25 share one walk per coldkey per fire AND
  // share the DB cache across cold lambdas. Pre-iter-127 each FY hit
  // /api/transfer/v1 separately with its own timestamp window, structurally
  // doubling the 429 surface.
  const allTransferRows = await getTransferRowsCached(coldkey);
  const transferRows = [];
  for (const t of allTransferRows) {
    ...
    if (tsMs == null || tsMs < effectiveStartMs || tsMs > endD.getTime()) continue;
    if (to === coldkey) {
      transferRows.push({
  ...
  for (let i = 0; i < snapshotsAsc.length; i++) {
    ...
    const row = {
      ...
      _source:
        i === 0
          ? (usedFallback
              ? 'free:history/v1@balanceTotal:fallback-oldest'
              : 'free:history/v1@balanceTotal:starting')
          : 'free:history/v1@balanceTotal:daily',
    };
    if (i > 0 && prevBalance != null) {
      const netT = transferNetByDate[row.date] || 0;
      row.daily_income = balance - prevBalance - netT;
    }
    out.push(row);
    prevBalance = balance;
  }
